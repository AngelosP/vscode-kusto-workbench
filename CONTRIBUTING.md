...
Keep provider tests focused on adapter responsibilities: message routing, manual SQL execution UX, database/schema I/O, persistence sanitation/publication, generic comparison fallback, panel transport, and Kusto behavior. Test dashboard workflows directly through `HostDashboardApplicationHandler`; use the real provider only to prove typed injection/forwarding and disposal. Protocol tests must inventory host messages emitted from `queryEditorProvider.ts`, `dashboardApplicationHandler.ts`, and `sql/sqlEditorLifecycleCoordinator.ts`.
