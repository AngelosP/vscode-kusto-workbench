...
3. **Keep exportable visual parity explicit.** HTML dashboard charts should use `KustoWorkbench.renderChart(bindingId)` in preview and provenance chart bindings for export. Exportable tables should use `KustoWorkbench.renderTable(bindingId)`, and table-cell visuals should live in provenance `columns[].cellBar` specs. Preview SVG/HTML and Power BI DAX/SVG should share the same spec, palette, geometry, ordering, top-N, label, and legend semantics.
...
7. **Document and test new binding shapes.** If adding scalar/table/pivot/chart display modes, table cell visuals, or `preAggregate` behavior, cover DAX generation and rendered HTML/SVG output in `powerBiExport.test.ts` and preview bridge behavior in webview tests.
