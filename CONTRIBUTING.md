...
Keep provider tests focused on adapter responsibilities: message routing, handler composition/forwarding, persistence sanitation/publication, panel transport, and Kusto behavior. Test SQL connection snapshots directly through `HostSqlConnectionsProjectionApplicationHandler`, SQL schema requests through `HostSqlSchemaRequestApplicationHandler`, manual SQL execution through `HostSqlSectionExecutionApplicationHandler`, and comparison preparation through `HostComparisonPreparationApplicationHandler`; use the real provider only for constructor ordering, reference-identical forwarding, exact settlement/rejection, zero direct effects, thin delegation where applicable, and disposal.
