...
Native `.kqlx` / `.sqlx` / `.mdx` Markdown, URL, Python, Chart, and Transformation persisted state have one host application owner. Preserve these boundaries:

* **Aggregate authority**: `MarkdownDocumentAggregate` owns ordered Markdown + URL + Python + Chart + Transformation application state, document revisions, section revisions, and add/patch/remove transitions. `markdownSectionDefinition.ts`, `urlSectionDefinition.ts`, `pythonSectionDefinition.ts`, `chartSectionDefinition.ts`, and `transformationSectionDefinition.ts` own persisted validation and patch semantics. Do not add a second per-kind aggregate, command queue, or authoritative state map.
...
* **Python runtime boundary**: only authored `name`, `code`, admitted terminal `output`, `expanded`, and `editorHeightPx` are aggregate state. Monaco instances, transient `Running...` UI, execution dispatch/process ownership, stdout/stderr acquisition, and local-code policy remain adapter effects. The box-ID-only protocol permits one outstanding request per section through `python-execution-admission.ts`; projection/removal/invalidation retires publication authority, inactive terminals settle without publishing, and the host must emit one bounded timeout terminal even if process close never arrives.
...
* **Complete projection admission**: Successful command results must match the full predicted owned projection: every Markdown/URL/Python/Chart/Transformation section, section revision, and exact mixed order. Inconsistent success reloads without adoption. Rejected results reconcile all five kinds and then run one stable whole-container order pass.
...
* **Panel handoff**: Retained aggregate state and live panel ownership are separate. Canonical-panel disposal reprojects a surviving panel without installing its historical owner or rewinding revisions. Same-document URL, Python, Chart, and Transformation moves retain runtime state, including URL artifacts, Python Monaco instances, Chart ECharts/artifact bindings, and Transformation input pins/lineage. Genuine removal tears down, and detached/replaced elements cannot persist or publish. Close cleanup rechecks live/closing panels atomically after queue settlement before deleting URI state.
...
* **Lossless boundary**: Apply aggregate Markdown + URL + Python + Chart + Transformation state to the final adapter snapshot, then serialize through `kqlxOverlay.ts` against the exact current source. Unknown root/state fields, known-section extensions, nested Chart settings, Transformation nested-array extensions, opaque sections, and order must survive; ambiguous nested correlation fails closed.

Focused coverage lives in `document-view-protocol.test.ts`, `webview-messages.test.ts`, `markdown-document-aggregate.test.ts`, `markdown-document-client.test.ts`, `kw-python-section.test.ts`, `kw-url-section.test.ts`, `chart-datasets.test.ts`, `chart-data-source.controller.test.ts`, `chart-renderer-zoom-pan.test.ts`, `kw-chart-section-agent.test.ts`, `transformation-join.test.ts`, `queryEditorProviderPython.test.ts`, `persistence-roundtrip.test.ts`, `message-handler.test.ts`, `message-protocol.test.ts`, `section-projection-order.test.ts`, `main-url-batch-visibility.test.ts`, `section-setName.test.ts`, `kqlxMarkdownOwnership.test.ts`, and the native `host-owned-markdown-lifecycle` scenarios. Run Vitest with `--maxWorkers=1` and extension-host tests with `--timeout 5000`.
