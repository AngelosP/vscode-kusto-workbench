...
Test Query Editor SQL schema requests directly through `HostSqlSchemaRequestApplicationHandler`; use the real provider for exact-object injection/forwarding, awaited settlement, exact rejection, zero direct lifecycle/service/transport effects, and disposal. Keep `SqlSchemaService`, cache-generation, lifecycle replay, real SQL router, Cached Values host/UI, and protocol inventory tests in the adjacent ring. Regressions must deliberately reverse same-target completion, supersede during disk CAS, fail stale-file cleanup, reuse an explicitly stale generation, rotate target/principal/privacy ownership, reject staged/applied publication, and exercise Clear All during pending snapshot/database/schema work. Run with `--maxWorkers=1`.
