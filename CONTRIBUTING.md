...
* **Persistence pairing**: `resultJson` remains the bounded row payload and `resultArtifact` is optional row-free identity/provenance metadata. Install restored descriptors only after owner/policy admission and accepted rendering. Every row deletion, privacy purge, or sanitation path must remove the descriptor atomically.
* **Migration scope**: Charts and transformations use immutable bindings; transformations are the first derived producer. Comparisons, dashboards, export, Copilot context, and exact SQL provenance still use mutable current pointers or the compatibility surface; migrate them one at a time with a discriminating pin/rebind/clear test rather than bypassing the facade.
