...
11. **Keep host/webview contracts typed and correlated.** Any new export/publish message must be added to both `queryEditorTypes.ts` and `webview-messages.ts`, carry exact request identity/cancellation, and be covered by `message-protocol.test.ts`. Tool-framework messages that intentionally use generic `toolResponse` still need protocol inventory coverage.
