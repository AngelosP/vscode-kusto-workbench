...
* **Migration scope**: Charts, transformations, Kusto/SQL comparisons, HTML preview bridges, Kusto model/tool result responses, Share Results, and Save Results CSV use immutable bindings or exact provenance. CSV requires its independent `exportToCsv` decision plus the nonce-correlated host protocol. Audit table-local copy and any remaining row consumers one at a time with a discriminating pin/rebind/clear test rather than bypassing the facade.
