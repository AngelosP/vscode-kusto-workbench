...
Focused coverage lives in `markdown-document-aggregate.test.ts`, `markdown-document-client.test.ts`, `kw-python-section.test.ts`, `kw-url-section.test.ts`, `chart-datasets.test.ts`, `chart-data-source.controller.test.ts`, `chart-renderer-zoom-pan.test.ts`, `kw-chart-section-agent.test.ts`, `transformation-join.test.ts`, `queryEditorProviderPython.test.ts`, `persistence-roundtrip.test.ts`, `message-handler.test.ts`, `section-projection-order.test.ts`, `main-url-batch-visibility.test.ts`, `section-setName.test.ts`, `kqlxMarkdownOwnership.test.ts`, and the native `host-owned-markdown-lifecycle` scenarios. Run Vitest with `--maxWorkers=1` and extension-host tests with `--timeout 5000`.
