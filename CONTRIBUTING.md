...
Keep provider tests focused on adapter responsibilities: message routing, manual SQL execution UX, database/schema I/O, persistence sanitation/publication, generic comparison fallback, panel transport, and Kusto behavior. Test dashboard workflows directly through `HostDashboardApplicationHandler`, governed native CSV workflows through `HostArtifactCsvSaveApplicationHandler`, imported CSV publication through `HostImportedCsvSaveApplicationHandler`, and Python process/terminal behavior through `HostPythonExecutionApplicationHandler`; use the real provider only to prove typed injection/forwarding and disposal. Protocol tests must inventory host messages emitted from `queryEditorProvider.ts`, `dashboardApplicationHandler.ts`, `artifactCsvSaveApplicationHandler.ts`, `pythonExecutionApplicationHandler.ts`, and `sql/sqlEditorLifecycleCoordinator.ts`.
