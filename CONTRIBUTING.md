...
* **Migration scope**: Charts, transformations, Kusto comparisons, and HTML preview bridges use immutable bindings and lineage. Export, Copilot/tool result context, SQL comparisons, and exact SQL provenance still use mutable current pointers or the compatibility surface; migrate them one at a time with a discriminating pin/rebind/clear test rather than bypassing the facade.
