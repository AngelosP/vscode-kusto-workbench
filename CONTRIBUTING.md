...
Test SQL single-connection onboarding directly through `HostSqlConnectionOnboardingApplicationHandler`, Query Editor SQL favorites directly through `HostSqlFavoritesApplicationHandler`, and Query Editor Kusto favorites directly through `HostKustoFavoritesApplicationHandler`; use the real provider only for exact-object injection/forwarding, projection/read delegation, activation, and disposal. Include all three handler files in host-message sender inventory whenever their transport changes.
