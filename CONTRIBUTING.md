...
* **Migration scope**: Charts are the first bound consumer. Transformations, comparisons, dashboards, export, Copilot context, and exact SQL provenance still use the mutable compatibility surface; migrate them one at a time with a discriminating pin/rebind/clear test rather than bypassing the facade.
