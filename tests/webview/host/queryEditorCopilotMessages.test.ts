...
function makeHost(): CopilotServiceHost {
	const host: any = {
		...
		getKustoSectionExecutionTarget: () => ({
			engine: 'kusto' as const, boxId: 'query_1', sectionInstanceId: 'instance-query_1', targetGeneration: 1,
			connectionId: 'connection-a', database: 'Db',
		}),
		executeKustoSectionQuery: async (options: any) => ({
			status: 'success' as const, executionId: options.executionId,
			result: { columns: [], rows: [], metadata: { cluster: '', database: '', executionTime: '' } },
		}),
		cancelKustoSectionExecution: () => true,
