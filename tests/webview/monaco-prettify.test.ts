...
describe('__kustoConvertFunctionToInline', () => {
	...
	it.each([
		['.create function after a line comment', '// helper used by Run Function\n.create function LeadingCreate() { T | count }', 'LeadingCreate'],
		['.create-or-alter function after a block comment', '/* helper used by Run Function */\n.create-or-alter function LeadingCreateOrAlter() { T | count }', 'LeadingCreateOrAlter'],
		['.alter function after a line comment', '// helper used by Run Function\n.alter function LeadingAlter() { T | count }', 'LeadingAlter'],
		['.create function after multiline block comment and blank line', '/*\n helper used by Run Function\n*/\n\n.create function LeadingBlockComment() { T | count }', 'LeadingBlockComment'],
		['.create-or-alter function after multiple line comments', '// helper used by Run Function\n// generated by Copilot\n.create-or-alter function LeadingLineComments() { T | count }', 'LeadingLineComments'],
		['real .create function after commented-out fake function', '// .create function Fake() { print fake=1 }\n.create function RealAfterFake() { T | count }', 'RealAfterFake'],
		['real .create-or-alter function after block-commented fake function', '/*\n.create function Fake() { print fake=1 }\n*/\n.create-or-alter function RealAfterBlockFake() { T | count }', 'RealAfterBlockFake'],
	])('converts %s', (_label, input, expectedName) => {
		const result = __kustoConvertFunctionToInline(input);
		expect(result).not.toBeNull();
		expect(result!.name).toBe(expectedName);
		expect(result!.text).toContain(`let ${expectedName} = () { T | count };`);
	});
	...
	it.each([
		['line comment .create', '// .create function Fake() { print x=1 }'],
		['line comment .create-or-alter', '// .create-or-alter function Fake() { print x=1 }'],
		['line comment .alter', '// .alter function Fake() { print x=1 }'],
		['inline block .create', '/* .create function Fake() { print x=1 } */'],
		['inline block .create-or-alter', '/* .create-or-alter function Fake() { print x=1 } */'],
		['inline block .alter', '/* .alter function Fake() { print x=1 } */'],
		['multiline block .create', '/*\n.create function Fake() { print x=1 }\n*/'],
		['multiline block .create-or-alter', '/*\n.create-or-alter function Fake() { print x=1 }\n*/'],
		['multiline block .alter', '/*\n.alter function Fake() { print x=1 }\n*/'],
	])('does not convert function definitions that are only inside comments: %s', (_label, input) => {
		expect(__kustoConvertFunctionToInline(input)).toBeNull();
	});
...
describe('__kustoParseFunction', () => {
	...
	it.each([
		...
		['real .create function after commented-out fake function', '// .create function Fake() { print fake=1 }\n.create function RealAfterFake() { T | count }', 'RealAfterFake'],
		['real .create-or-alter function after block-commented fake function', '/*\n.create function Fake() { print fake=1 }\n*/\n.create-or-alter function RealAfterBlockFake() { T | count }', 'RealAfterBlockFake'],
	])('parses %s', (_label, input, expectedName) => {
		const result = __kustoParseFunction(input);
		expect(result).not.toBeNull();
		expect(result!.name).toBe(expectedName);
		expect(result!.rawParams).toBe('');
		expect(result!.body).toBe(' T | count ');
	});
	...
	it('parses default parameter values that contain nested braces before the function body', () => {
		const result = __kustoParseFunction('.create function MyFunc(payload:dynamic = dynamic({"k":"v"})) { print payload }');
		expect(result).not.toBeNull();
		expect(result!.name).toBe('MyFunc');
		expect(result!.rawParams).toBe('payload:dynamic = dynamic({"k":"v"})');
		expect(result!.body).toBe(' print payload ');
	});
	...
	it.each([
		...
		['multiline block .create-or-alter', '/*\n.create-or-alter function Fake() { print x=1 }\n*/'],
		['multiline block .alter', '/*\n.alter function Fake() { print x=1 }\n*/'],
	])('returns null for function definitions that are only inside comments: %s', (_label, input) => {
		expect(__kustoParseFunction(input)).toBeNull();
	});
...
describe('__kustoHasFunctionDefinition', () => {
	...
	it.each([
		['block comment before .create-or-alter', '/* helper */\n.create-or-alter function MyFunc() { T }'],
		['multiline block comment and blank line before .create', '/*\nhelper\n*/\n\n.create function MyFunc() { T }'],
		['multiple line comments before .create-or-alter', '// helper\n// generated\n.create-or-alter function MyFunc() { T }'],
		['line-commented fake before real .create', '// .create function Fake() { T }\n.create function MyFunc() { T }'],
		['block-commented fake before real .create-or-alter', '/* .create function Fake() { T } */\n.create-or-alter function MyFunc() { T }'],
		['multiline-block-commented fake before real .alter', '/*\n.create function Fake() { T }\n*/\n.alter function MyFunc() { T }'],
	])('returns true for function definition after %s', (_label, input) => {
		expect(__kustoHasFunctionDefinition(input)).toBe(true);
	});
	...
	it.each([
		...
		['multiline block .create-or-alter', '/*\n.create-or-alter function Fake() { print x=1 }\n*/'],
		['multiline block .alter', '/*\n.alter function Fake() { print x=1 }\n*/'],
	])('returns false for function definitions that are only inside comments: %s', (_label, input) => {
		expect(__kustoHasFunctionDefinition(input)).toBe(false);
	});
