Feature: Kusto toolbar, dropdown, dialog, and Copilot UX
  ...
  Scenario: Menus, dropdowns, dialogs, chat, and section controls preserve focus and dismiss correctly
    ...
    When I evaluate "(() => { const chat = document.querySelector('kw-copilot-chat'); const panel = chat?.shadowRoot?.querySelector('.tools-panel'); if (!panel || panel.getBoundingClientRect().width <= 0) throw new Error('Copilot tools panel did not open visibly'); return 'copilot tools open'; })()" in the webview
