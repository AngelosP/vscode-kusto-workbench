Feature: Kusto query sections preserve multiple primary results
  ...
  Scenario: Run All with standalone separators, select Result 2, chart it, save, and reopen
    ...
    When I evaluate "(() => window.addTransformationBox({ id: 'transformation_multi_result_update', name: 'Updated indexed join', dataSourceId: 'query_multi_result', dataSourceResultIndex: 0, transformationType: 'join', joinRightDataSourceId: 'query_multi_result', joinRightDataSourceResultIndex: 0, joinKind: 'fullouter', joinKeys: [{ left: 'Value', right: 'Value' }], mode: 'preview', expanded: true, afterBoxId: 'chart_multi_result_two' }))()" in the webview "multi-result.kqlx"
    And I wait 3 seconds
    When I evaluate "(() => { const section = document.getElementById('transformation_multi_result_update'); if (!section || section.configure({ dataSourceResultIndex: 1, joinRightDataSourceResultIndex: 2 }) !== true) throw new Error('Existing transformation update was rejected'); section.commitDocumentState(); return section.serialize(); })()" in the webview "multi-result.kqlx"
    And I wait 3 seconds
    Then I collect JSON artifact "updated-transformation-indexes" from webview expression "(() => { const state = document.getElementById('transformation_multi_result_update')?.serialize?.(); if (state?.dataSourceResultIndex !== 1 || state?.joinRightDataSourceResultIndex !== 2) throw new Error('Updated transformation indexes were not applied'); return { dataSourceResultIndex: state.dataSourceResultIndex, joinRightDataSourceResultIndex: state.joinRightDataSourceResultIndex }; })()"

    When I execute command "workbench.action.files.save"
    And I wait 3 seconds
    Then I collect JSON artifact "durable-multi-result-file" from extension host expression "(async () => { const raw = value => value && typeof value === 'object' ? value.full ?? value.display : value; const suffix = '/tests/vscode-extension-tester/runs/kusto-auth/kusto-multi-result/multi-result.kqlx'; const document = vscode.workspace.textDocuments.find(candidate => candidate.uri.path.replace(/\\/g, '/').endsWith(suffix)); if (!document) throw new Error('Multi-result document is not open'); const deadline = Date.now() + 10000; while (Date.now() < deadline) { const text = new TextDecoder().decode(await vscode.workspace.fs.readFile(document.uri)); const file = JSON.parse(text); const query = file.state.sections.find(section => section.id === 'query_multi_result'); const chart = file.state.sections.find(section => section.id === 'chart_multi_result_two'); const transformation = file.state.sections.find(section => section.id === 'transformation_multi_result_update'); const batch = query?.resultJson ? JSON.parse(query.resultJson) : undefined; const second = batch?.additionalResults?.sets?.[0]; const secondRow = second?.rows?.[0]?.map(raw); const resultSetCount = 1 + (batch?.additionalResults?.sets?.length || 0); if (query?.selectedResultIndex === 1 && query?.resultArtifact?.version === 1 && resultSetCount === 3 && second?.resultIndex === 1 && secondRow?.[0] === 'second' && Number(secondRow?.[1]) === 2 && chart?.dataSourceResultIndex === 1 && transformation?.dataSourceResultIndex === 1 && transformation?.joinRightDataSourceResultIndex === 2) return { selectedResultIndex: query.selectedResultIndex, artifactId: query.resultArtifact.artifactId, resultSetCount, secondRow, chartResultIndex: chart.dataSourceResultIndex, transformationResultIndex: transformation.dataSourceResultIndex, transformationJoinRightResultIndex: transformation.joinRightDataSourceResultIndex }; await new Promise(resolve => setTimeout(resolve, 100)); } throw new Error('Durable multi-result state did not settle'); })()"
    ...
    Then I collect JSON artifact "reopened-multi-result" from webview expression "(() => { const raw = value => value && typeof value === 'object' ? value.full ?? value.display : value; const table = document.getElementById('query_multi_result').querySelector('kw-data-table'); const picker = table.shadowRoot.querySelector('[data-testid=result-set-picker]'); const chart = document.getElementById('chart_multi_result_two').serialize(); const transformation = document.getElementById('transformation_multi_result_update')?.serialize?.(); if (transformation?.dataSourceResultIndex !== 1 || transformation?.joinRightDataSourceResultIndex !== 2) throw new Error('Reopened transformation indexes regressed'); return { selectedResultIndex: Number(picker.value), row: table.rows[0].map(raw), chartResultIndex: chart.dataSourceResultIndex, transformationResultIndex: transformation.dataSourceResultIndex, transformationJoinRightResultIndex: transformation.joinRightDataSourceResultIndex }; })()"
    When I execute command "workbench.action.focusActiveEditorGroup"
    And I move the mouse to 30, 700
    And I click
    And I press "Escape"
