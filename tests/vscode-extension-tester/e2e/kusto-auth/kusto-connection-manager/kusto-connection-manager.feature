Feature: Connection Manager with Kusto connections — explorer, drill-down, favorites
  ...
  Scenario: Connection list, drill into cluster, explore databases
    ...
    When I evaluate "(() => { const modal = document.querySelector('kw-connection-manager')?.shadowRoot?.querySelector('[data-testid=cm-modal-overlay]'); if (!modal) throw new Error('Edit modal did not open'); const h2 = modal.querySelector('h2'); if (!h2?.textContent?.includes('Edit')) throw new Error('Not an Edit modal: ' + h2?.textContent); const form = modal.querySelector('kw-kusto-connection-form'); if (!form) throw new Error('No form in Edit modal'); const testButton = form.shadowRoot?.querySelector('[data-testid=kusto-conn-test]'); if (!testButton || !testButton.textContent?.includes('Test Connection')) throw new Error('Edit modal missing Test Connection button'); return 'edit modal opened with Test Connection: ' + h2.textContent; })()" in the webview
