Feature: Session results survive an Extension Host restart
  ...
  Scenario: Durable Kusto results return when session.kqlx reopens after window reload
    ...
    Then I collect JSON artifact "reopened-session-result" from webview expression "(() => { const raw = value => value && typeof value === 'object' ? value.full ?? value.display : value; const section = document.getElementById('query_persisted_results'); const table = section?.querySelector('kw-data-table'); const columns = (table?.columns || []).map(column => typeof column === 'string' ? column : column.name); const row = table?.rows?.[0] || []; if (Number(raw(row[columns.indexOf('RowId')])) !== 1 || raw(row[columns.indexOf('Label')]) !== 'persist_row_01') throw new Error('Reopened session result is missing: ' + JSON.stringify({ columns, row: row.map(raw) })); return { columns, row: row.map(raw), artifactId: table.resultArtifactId }; })()"
