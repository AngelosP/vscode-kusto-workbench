Feature: Kusto Connection Manager surrounding UX
  ...
  Scenario: Add and edit dialogs preserve focus and never save cancelled drafts
    ...
    When I click "[data-testid='cm-add-connection']" in the webview "Connection Manager"
    When I wait for "[data-testid='cm-modal-save']" in the webview "Connection Manager" for 10 seconds
    When I evaluate "(() => { const root = document.querySelector('kw-connection-manager')?.shadowRoot; const dialog = root?.querySelector('[data-testid=cm-modal-overlay]'); const body = root?.querySelector('.modal-body'); const footer = root?.querySelector('.modal-footer'); const dialogRect = dialog?.getBoundingClientRect(); const footerRect = footer?.getBoundingClientRect(); if (!dialog?.open || !dialogRect || dialogRect.top < 0 || dialogRect.bottom > innerHeight || getComputedStyle(body).overflowY !== 'auto' || body.scrollHeight <= body.clientHeight || !footerRect || footerRect.top < dialogRect.top || footerRect.bottom > dialogRect.bottom) throw new Error('Short modal is clipped or lacks a scrollable body: ' + JSON.stringify({ innerHeight, dialog: dialogRect && { top: dialogRect.top, bottom: dialogRect.bottom }, body: body && { clientHeight: body.clientHeight, scrollHeight: body.scrollHeight, overflowY: getComputedStyle(body).overflowY }, footer: footerRect && { top: footerRect.top, bottom: footerRect.bottom } })); body.scrollTop = body.scrollHeight; return { dialogHeight: dialogRect.height, bodyClientHeight: body.clientHeight, bodyScrollHeight: body.scrollHeight, bodyScrollTop: body.scrollTop }; })()" in the webview "Connection Manager"
    Then I take a screenshot "06-manager-short-dialog"
    When I press "Escape"
    When I evaluate "(() => { const root = document.querySelector('kw-connection-manager')?.shadowRoot; if (root?.querySelector('.modal-overlay, .modal-content, kw-kusto-connection-form')) throw new Error('Short modal did not close cleanly'); return 'short modal dismissed'; })()" in the webview "Connection Manager"
