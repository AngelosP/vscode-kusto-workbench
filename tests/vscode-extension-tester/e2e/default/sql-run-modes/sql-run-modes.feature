Feature: SQL run modes - split button, plain vs TOP 100
  ...
  Scenario: Run mode defaults to TOP 100, can switch modes, label updates without a SQL connection
    ...
    When I evaluate "(() => { const el = document.querySelector('kw-sql-section'); if (!el) throw new Error('SQL section not found'); const data = el.serialize(); if (Object.prototype.hasOwnProperty.call(data, 'runMode')) throw new Error('Default top100 runMode should be omitted, got: ' + JSON.stringify(data.runMode)); return 'default top100 runMode omitted from serialization'; })()" in the webview
    ...
    When I evaluate "(() => { const el = document.querySelector('kw-sql-section'); if (!el) throw new Error('SQL section not found'); const data = el.serialize(); if (!Object.prototype.hasOwnProperty.call(data, 'runMode') || data.runMode !== 'plain') throw new Error('Nondefault plain runMode should serialize explicitly, got: ' + JSON.stringify(data.runMode)); return 'nondefault runMode serialized as plain'; })()" in the webview
    ...
    When I evaluate "(() => { const el = document.querySelector('kw-sql-section'); if (!el) throw new Error('SQL section not found'); const data = el.serialize(); if (Object.prototype.hasOwnProperty.call(data, 'runMode')) throw new Error('Restored default top100 runMode should be omitted, got: ' + JSON.stringify(data.runMode)); return 'restored default top100 runMode omitted from serialization'; })()" in the webview
