Feature: Inline CSS — webview is styled at first paint with no external CSS dependency
  ...
  Scenario: Query editor webview has inline CSS, not an external link
    ...
    # ── TEST 4: Theme typography/foreground apply and an editor surface is opaque ──
    When I evaluate "(() => { const bodyStyle = getComputedStyle(document.body); const themeFont = bodyStyle.getPropertyValue('--vscode-font-family').trim(); const themeForeground = bodyStyle.getPropertyValue('--vscode-foreground').trim(); const fontFamily = bodyStyle.fontFamily; const color = bodyStyle.color; const isTransparent = value => value === 'transparent' || value.split(' ').join('') === 'rgba(0,0,0,0)'; if (!themeFont) throw new Error('--vscode-font-family is missing'); if (!themeForeground) throw new Error('--vscode-foreground is missing'); if (!fontFamily || fontFamily.toLowerCase().includes('times new roman') || fontFamily.toLowerCase() === 'serif') throw new Error('Body font not themed: ' + fontFamily); if (!color || isTransparent(color)) throw new Error('Body foreground not themed: ' + color); const surface = document.querySelector('.query-editor-wrapper'); if (!surface) throw new Error('No editor surface found'); const surfaceBg = getComputedStyle(surface).backgroundColor; if (isTransparent(surfaceBg)) throw new Error('Editor surface background is transparent: ' + surfaceBg); return 'theme styling verified ✓ color=' + color + ' font=' + fontFamily.substring(0, 40) + ' surface=' + surfaceBg; })()" in the webview
