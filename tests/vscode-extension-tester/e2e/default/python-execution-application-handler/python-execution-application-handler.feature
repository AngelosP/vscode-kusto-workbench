Feature: Python execution application handler
  ...
  Scenario: Python execution renders and persists exact host output
    ...
    When I evaluate "(async () => { const section = document.getElementById('python_hst3'); const deadline = Date.now() + 20000; while (Date.now() < deadline) { await section.updateComplete; const state = section.createDocumentState(); const output = section.shadowRoot?.querySelector('.python-output')?.textContent ?? ''; if (state.output.trim() === 'HST3:24' && output.trim() === 'HST3:24' && !section.shadowRoot?.querySelector('.run-btn')?.disabled) return { output, rawStateOutput: state.output, code: state.code, running: false }; await new Promise(resolve => setTimeout(resolve, 100)); } throw new Error('Exact Python output did not arrive: ' + JSON.stringify(section.createDocumentState())); })()" in the webview for 25 seconds
    When I execute command "workbench.action.files.save"
    Then I collect JSON artifact "hst3-python-execution" from extension host expression "(async () => { const suffix = '/tests/vscode-extension-tester/runs/default/python-execution-application-handler/python-execution.kqlx'; const expectedOutput = 'HST3:24' + (process.platform === 'win32' ? String.fromCharCode(13, 10) : String.fromCharCode(10)); const deadline = Date.now() + 10000; while (Date.now() < deadline) { const document = vscode.workspace.textDocuments.find(candidate => candidate.uri.path.replace(/\\/g, '/').endsWith(suffix)); if (document && !document.isDirty) { const file = JSON.parse(document.getText()); const python = file.state.sections.find(section => section.id === 'python_hst3'); if (python?.code?.includes('sum([7, 8, 9])') !== true || python?.output !== expectedOutput) throw new Error('Host buffer lost exact Python execution state: ' + JSON.stringify(python)); return { code: python.code, output: python.output, normalizedOutput: python.output.trim(), dirty: document.isDirty }; } await new Promise(resolve => setTimeout(resolve, 100)); } throw new Error('Python execution Save did not settle'); })()"
    When I execute command "workbench.action.closeAllEditors"
    When I open file "tests/vscode-extension-tester/runs/default/python-execution-application-handler/python-execution.kqlx" in the editor
    When I wait for "#python_hst3" in the webview for 20 seconds
    When I evaluate "(async () => { const section = document.getElementById('python_hst3'); await section.updateComplete; const deadline = Date.now() + 10000; while (Date.now() < deadline) { const state = section.createDocumentState(); const output = section.shadowRoot?.querySelector('.python-output')?.textContent ?? ''; if (state.output.trim() === 'HST3:24' && output.trim() === 'HST3:24') return { output, rawStateOutput: state.output, code: state.code }; await new Promise(resolve => setTimeout(resolve, 100)); } throw new Error('Persisted Python output did not restore'); })()" in the webview for 15 seconds
