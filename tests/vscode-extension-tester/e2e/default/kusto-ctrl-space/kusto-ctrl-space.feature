Feature: Kusto Ctrl+Space autocomplete shortcut
  ...
  Scenario: Ctrl+Space opens Kusto suggestions in the real webview editor
    ...
    When I evaluate "window.__e2e.suggest.kusto.waitExistingAllVisible('manual Ctrl+Space Kusto suggestions at the strict pipe caret', '', 5000)" in the webview
    When I evaluate "(() => { const trace = window.__e2e.kusto.compactAutocompleteTrace(); const position = trace?.seed?.position; if (position?.lineNumber !== 2 || position?.column !== 3 || !trace?.events?.some(event => event.event === 'suggest-triggered')) throw new Error('Ctrl+Space did not use the strict pipe caret: ' + JSON.stringify(trace)); return 'Ctrl+Space used strict pipe caret 2:3'; })()" in the webview
  ...
  Scenario: Ctrl+Space opens suggestions inside a function over a fully qualified remote table
    ...
    When I evaluate "(() => { const section = document.querySelector('kw-query-section'); const editor = window.queryEditors?.[section?.boxId || section?.id]; const query = String(editor?.getValue?.() || ''); const marked = query.replace('| where TIMESTAMP >=', '| where TIME⟦caret⟧STAMP >='); if (!query || marked === query) throw new Error('Could not restore the first TIMESTAMP caret'); return window.__e2e.kusto.setQueryWithCaretMarkerStrict(marked); })()" in the webview
    And I wait 1 second
    When I evaluate "window.__e2e.suggest.kusto.hide()" in the webview
    When I evaluate "window.__e2e.suggest.kusto.assertHidden('after setting semantic scenario before Ctrl+Space')" in the webview
    When I press "Ctrl+Space"
    When I evaluate "window.__e2e.suggest.kusto.waitRenderedAllColumnsVisible('FQ function Ctrl+Space rendered columns', 'TIMESTAMP,TIMESTAMP_RemoteOnly', 10000)" in the webview for 15 seconds
    When I evaluate "window.__e2e.suggest.kusto.assertAllRenderedSnapshotsHaveColumns('FQ function Ctrl+Space remains visible', 'TIMESTAMP,TIMESTAMP_RemoteOnly', 1000, 100)" in the webview for 5 seconds
  ...
  Scenario: Ctrl+Space loads a fully qualified remote schema without a primary selection
    ...
    When I click "button[data-add-kind='query']" in the webview
    When I wait for "kw-query-section" in the webview for 15 seconds
    When I evaluate "(async () => { const el = document.querySelector('kw-query-section'); if (!el) throw new Error('Kusto section missing'); el.setConnectionId(''); el.setDatabase(''); await el.updateComplete; if (el.getConnectionId?.() || el.getDatabase?.()) throw new Error('Expected no primary Kusto selection'); if (!Array.isArray(window.connections) || !window.connections.some(connection => String(connection.clusterUrl || '').includes('supplemental-remote.westus'))) throw new Error('Seeded remote connection is unavailable for identity resolution'); return 'no primary selection with seeded remote identity'; })()" in the webview
    When I click "kw-query-section .query-editor" in the webview
    ...
    When I evaluate "window.__e2e.suggest.kusto.hide()" in the webview
    When I press "Ctrl+Space"
    When I evaluate "window.__e2e.suggest.kusto.waitRenderedAllColumnsVisible('no-primary fully qualified Ctrl+Space', 'RemoteOnlyColumn', 10000)" in the webview for 15 seconds
    When I execute command "workbench.action.closeAllEditors"
    When I execute command "kustoWorkbench.test.cleanupSupplementalSchemaDiagnosticsState"
  ...
  Scenario: A cold no-context schema automatically retries the original caret
    ...
    When I evaluate "window.__e2e.suggest.kusto.waitExistingAllVisible('cold no-context fallback while schema loads', 'abs,acos', 2000)" in the webview for 5 seconds
  ...
  Scenario: Dismissing a cold fallback cancels its semantic retry
    ...
    When I evaluate "window.__e2e.suggest.kusto.hide()" in the webview
    When I evaluate "window.__e2e.kusto.triggerSuggest()" in the webview for 10 seconds
    When I evaluate "window.__e2e.suggest.kusto.waitExistingAllVisible('cold fallback before dismissal', 'abs,acos', 2000)" in the webview for 5 seconds
    When I press "Escape"
  ...
  Scenario: Ctrl+Space falls back after a missing remote schema timeout
    ...
    When I evaluate "window.__e2e.kusto.applySemanticCompletionFixture()" in the webview
    When I evaluate "window.__e2e.kusto.setQueryWithCaretMarkerStrict('LocalOnly\n| union cluster(\'missingremote\').database(\'missingdb\').MissingTable\n| ⟦caret⟧')" in the webview
    And I wait 1 second
    When I evaluate "window.__e2e.suggest.kusto.hide()" in the webview
    When I evaluate "window.__e2e.suggest.kusto.assertHidden('before missing remote Ctrl+Space')" in the webview
    When I press "Ctrl+Space"
    When I evaluate "window.__e2e.suggest.kusto.waitVisible('missing remote schema fallback after bounded wait', 'where,project', 4000)" in the webview
