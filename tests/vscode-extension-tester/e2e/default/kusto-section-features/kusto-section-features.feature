Feature: Kusto section features — toolbar, run modes, persistence without connection
  ...
  Scenario: Toolbar actions, run modes, and section serialization
    ...
    When I evaluate "(() => { const sections = Array.from(document.querySelectorAll('kw-query-section')); if (sections.length !== 1) throw new Error('Expected exactly 1 KQL section, got ' + sections.length); const el = sections[0]; const toggle = el.querySelector('#' + CSS.escape(el.boxId + '_run_toggle')); if (!toggle) throw new Error('Run split toggle not found in kw-query-section light DOM'); return 'run split toggle found in light DOM ✓'; })()" in the webview
