Feature: Persistence contracts for unresolved selections and legacy file shapes
  ...
  Scenario: Save and reopen unresolved Kusto and SQL selections without dropping metadata
    ...
    When I evaluate "window.__e2e.workbench.clearSections()" in the webview "unresolved-selection.kqlx"
    And I wait 2 seconds

    When I evaluate "(() => { const queryId = window.addQueryBox({ id: 'query_unresolved_selection', initialQuery: String.raw`print marker = 'unresolved_kusto_marker'` }); const query = document.getElementById(queryId); query.setName('Unresolved Kusto Selection'); query.setDesiredClusterUrl('https://unresolved-e2e.kusto.windows.net'); query.setDesiredDatabase('UnresolvedDb'); window.addSqlBox({ id: 'sql_unresolved_selection', name: 'Unresolved SQL Selection', query: String.raw`SELECT 'unresolved_sql_marker' AS marker;`, serverUrl: 'unresolved-sql-e2e.database.windows.net', database: 'UnresolvedWarehouse', expanded: true, afterBoxId: queryId }); return 'created unresolved selection sections'; })()" in the webview "unresolved-selection.kqlx"
    And I wait 5 seconds

    When I evaluate "window.__e2e.persistence.selectKustoRunMode('sample100', '#query_unresolved_selection')" in the webview "unresolved-selection.kqlx"
    When I evaluate "window.__e2e.persistence.selectSqlRunMode('plain', 'sql_unresolved_selection')" in the webview "unresolved-selection.kqlx"
    And I wait 1 second

    When I evaluate "window.__e2e.persistence.assertDocumentKind('kqlx')" in the webview "unresolved-selection.kqlx"
    When I evaluate "window.__e2e.persistence.assertSectionOrder('query,sql')" in the webview "unresolved-selection.kqlx"
    When I evaluate "window.__e2e.persistence.assertSectionIds('query_unresolved_selection,sql_unresolved_selection')" in the webview "unresolved-selection.kqlx"
    When I evaluate "window.__e2e.persistence.assertQuerySection('query_unresolved_selection', { name: 'Unresolved Kusto Selection', queryIncludes: 'unresolved_kusto_marker', clusterUrl: 'https://unresolved-e2e.kusto.windows.net', database: 'UnresolvedDb', runMode: 'sample100', resultsVisible: true })" in the webview "unresolved-selection.kqlx"
    When I evaluate "window.__e2e.persistence.assertSqlSection('sql_unresolved_selection', { name: 'Unresolved SQL Selection', queryIncludes: 'unresolved_sql_marker', serverUrl: 'unresolved-sql-e2e.database.windows.net', database: 'UnresolvedWarehouse', runMode: 'plain' })" in the webview "unresolved-selection.kqlx"
    ...
    When I open file "tests/vscode-extension-tester/runs/default/persistence-contracts/unresolved-selection.kqlx" in the editor
    And I wait 8 seconds
    When I wait for "kw-sql-section" in the webview "unresolved-selection.kqlx" for 20 seconds
  ...
  Scenario: Legacy KQLX file shapes restore as current sections
    ...
    And I wait 8 seconds
    When I wait for "kw-query-section" in the webview "legacy-kqlx-contract.kqlx" for 20 seconds
    When I wait for "kw-markdown-section" in the webview "legacy-kqlx-contract.kqlx" for 20 seconds
    And I wait 4 seconds
    When I execute command "workbench.action.focusActiveEditorGroup"
    And I move the mouse to 30, 700
    And I click
    ...
    When I evaluate "(() => { if (!window.__e2e?.persistence) throw new Error('Persistence E2E bridge unavailable'); return window.__e2e.persistence.assertDocumentKind('kqlx'); })()" in the webview "legacy-kqlx-contract.kqlx"
    When I evaluate "(() => { if (!window.__e2e?.persistence) throw new Error('Persistence E2E bridge unavailable'); return window.__e2e.persistence.assertSectionOrder('query,markdown'); })()" in the webview "legacy-kqlx-contract.kqlx"
    When I evaluate "(() => { if (!window.__e2e?.persistence) throw new Error('Persistence E2E bridge unavailable'); return window.__e2e.persistence.assertSectionIds('query_legacy_copilot,markdown_legacy_preview'); })()" in the webview "legacy-kqlx-contract.kqlx"
    When I evaluate "(() => { if (!window.__e2e?.persistence) throw new Error('Persistence E2E bridge unavailable'); return window.__e2e.persistence.assertQuerySection('query_legacy_copilot', { name: 'Legacy Copilot Query', queryIncludes: 'legacy_kqlx_marker_alpha', clusterUrl: 'https://persisted-results-e2e.kusto.windows.net', database: 'PersistedDb', runMode: 'sample100', resultsVisible: false, resultRows: 2, resultColumns: 'RowId,Label' }); })()" in the webview "legacy-kqlx-contract.kqlx"
    When I evaluate "(() => { if (!window.__e2e?.persistence) throw new Error('Persistence E2E bridge unavailable'); return window.__e2e.persistence.assertMarkdownSection('markdown_legacy_preview', { title: 'Legacy Preview Notes', textIncludes: 'legacy markdown marker', mode: 'preview', tab: 'preview' }); })()" in the webview "legacy-kqlx-contract.kqlx"
