Feature: Connection Manager — Back button, filter bar, and Search tab
  ...
  Scenario: Filter bar always visible with Search tab, back button, and search UI
    ...
    # ── TEST 6: Search container has input, categories, and refresh split ─
    When I evaluate "(() => { const sr = document.querySelector('kw-connection-manager')?.shadowRoot; const input = sr?.querySelector('[data-testid=cm-search-input]'); if (!input) throw new Error('Search input not found'); const cats = sr?.querySelector('[data-testid=cm-search-categories]'); if (!cats) throw new Error('Categories container not found'); const chips = cats.querySelectorAll('.search-category-chip'); if (chips.length !== 4) throw new Error('Expected 4 Kusto category chips, got ' + chips.length); const split = sr?.querySelector('.search-refresh-split'); const refresh = split?.querySelector('.search-refresh-main'); const menuButton = split?.querySelector('.search-refresh-drop'); if (!refresh || !menuButton) throw new Error('Refresh split controls not found'); return 'input + ' + chips.length + ' categories + refresh split'; })()" in the webview
    ...
    # ── TEST 9: Tables pill cycles names → content → off → names ──────────
    When I evaluate "(async () => { const cm = document.querySelector('kw-connection-manager'); const sr = cm?.shadowRoot; if (!cm || !sr) throw new Error('Connection manager missing'); const findTables = () => Array.from(sr.querySelectorAll('[data-testid=cm-search-categories] .search-category-chip')).find(c => c.textContent.trim().includes('Table')); let tablesChip = findTables(); if (!tablesChip?.textContent.trim().includes('Table Names')) throw new Error('Initial Tables chip should show Table Names, got: ' + tablesChip?.textContent.trim()); tablesChip.click(); await cm.updateComplete; tablesChip = findTables(); if (!tablesChip?.classList.contains('content-on') || !tablesChip.textContent.trim().includes('Table Names & Columns')) throw new Error('Content state should show Table Names & Columns, got: ' + tablesChip?.textContent.trim()); tablesChip.click(); await cm.updateComplete; tablesChip = findTables(); if (tablesChip?.classList.contains('active')) throw new Error('Tables chip should be off after second click'); tablesChip?.click(); await cm.updateComplete; tablesChip = findTables(); if (!tablesChip?.classList.contains('active') || !tablesChip.textContent.trim().includes('Table Names')) throw new Error('Tables chip should return to names-only state'); return 'Tables pill 3-state cycle works'; })()" in the webview

    # ── TEST 10: Refresh split opens both current refresh choices ─────────
    When I evaluate "(async () => { const cm = document.querySelector('kw-connection-manager'); const sr = cm?.shadowRoot; if (!cm || !sr) throw new Error('Connection manager missing'); const main = sr.querySelector('.search-refresh-main'); const drop = sr.querySelector('.search-refresh-drop'); if (!main || !drop) throw new Error('Refresh split controls missing'); const mainText = main.textContent.trim().replace(/\s+/g, ' '); if (!mainText.includes('Refresh schemas') || !mainText.includes('(0)')) throw new Error('Unexpected refresh main text: ' + mainText); drop.click(); await cm.updateComplete; const items = Array.from(sr.querySelectorAll('.search-refresh-menu .search-tools-item-title')).map(item => item.textContent.trim().replace(/\s+/g, ' ')); const expected = ['Refresh connections with cached schemas (0)', 'Refresh all connections (0)']; if (JSON.stringify(items) !== JSON.stringify(expected)) throw new Error('Unexpected refresh menu items: ' + JSON.stringify(items)); return 'refresh menu: ' + items.join(' | '); })()" in the webview
    Then I take a screenshot "06-refresh-menu"

    # ── TEST 11: Refresh split closes without starting a refresh ──────────
    When I evaluate "(async () => { const cm = document.querySelector('kw-connection-manager'); const sr = cm?.shadowRoot; if (!cm || !sr) throw new Error('Connection manager missing'); const drop = sr.querySelector('.search-refresh-drop'); if (!drop) throw new Error('Refresh menu button missing'); drop.click(); await cm.updateComplete; if (sr.querySelector('.search-refresh-menu')) throw new Error('Refresh menu should close on second click'); return 'refresh menu closed'; })()" in the webview
    ...
    And I wait 1 second
    When I evaluate "(() => { const sr = document.querySelector('kw-connection-manager')?.shadowRoot; const cats = sr?.querySelector('[data-testid=cm-search-categories]'); const chips = cats?.querySelectorAll('.search-category-chip') || []; const labels = Array.from(chips).map(c => c.textContent.trim().replace(/\s+/g, ' ')); const expected = ['Servers', 'Databases', 'Table Names & Columns', 'View Names & Columns', 'Stored Proc Names & Body']; if (JSON.stringify(labels) !== JSON.stringify(expected)) throw new Error('Unexpected SQL categories: ' + JSON.stringify(labels)); return '5 SQL categories: ' + labels.join(', '); })()" in the webview
    ...
    When I evaluate "(() => { const sr = document.querySelector('kw-connection-manager')?.shadowRoot; const chips = sr?.querySelectorAll('[data-testid=cm-search-categories] .search-category-chip.has-content') || []; if (chips.length !== 3) throw new Error('Expected 3 SQL pills with content toggle, got ' + chips.length); const texts = Array.from(chips).map(c => c.querySelector('.search-chip-text')?.textContent?.trim().replace(/\s+/g, ' ')); const expected = ['Table Names & Columns', 'View Names & Columns', 'Stored Proc Names & Body']; if (JSON.stringify(texts) !== JSON.stringify(expected)) throw new Error('Unexpected SQL expandable labels: ' + JSON.stringify(texts)); return 'SQL expandable pills: ' + texts.join(', '); })()" in the webview
    ...
    When I evaluate "(async () => { const cm = document.querySelector('kw-connection-manager'); if (!cm) throw new Error('Connection manager missing'); cm._activeFilter = 'all'; cm._snapshot = { ...cm._snapshot || {}, connections: [{ id: 'test1', name: 'TestCluster', clusterUrl: 'https://test.kusto.windows.net', database: '' }], cachedDatabases: { test1: ['db1', 'db2'] }, favorites: [], leaveNoTraceClusters: [] }; cm._explorerPath = { connectionId: 'test1' }; cm.requestUpdate(); await cm.updateComplete; await new Promise(resolve => requestAnimationFrame(() => requestAnimationFrame(resolve))); return 'injected connection + drilled to cluster'; })()" in the webview
    And I wait 1 second
