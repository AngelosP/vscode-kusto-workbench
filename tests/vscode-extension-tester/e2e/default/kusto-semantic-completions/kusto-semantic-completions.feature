Feature: Kusto semantic autocomplete with deterministic schemas
  ...
  Scenario: Current-cluster synthetic function where dropdown renders TIMESTAMP column
    ...
    When I evaluate "window.__e2e.kusto.assertAutocompleteTraceEvents('current cluster synthetic function trace', 'trigger-start,schema-prepare-context,schema-prepare-refs,schema-prepare-primary-wait,schema-prepare-result,suggest-triggered,trace-finish', 'schema-prepare-cross-cluster-wait')" in the webview
