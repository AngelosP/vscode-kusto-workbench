Feature: Legacy persisted Kusto result migration
  ...
  Scenario: Markerless result is adopted once and powers its chart after reopen
    ...
    When I evaluate "window.__e2e.workbench.waitForPersistedResult('query_legacy_migration', 19000)" in the webview for 20 seconds
    Then I collect JSON artifact "legacy-migration-first-chart" from webview expression "window.__e2e.workbench.assertMigratedResultChart('query_legacy_migration', 'chart_legacy_migration', 3)"
    ...
    When I open file "tests/vscode-extension-tester/runs/default/legacy-result-migration/legacy-chart.kqlx" in the editor
    When I evaluate "window.__e2e.workbench.waitForPersistedResult('query_legacy_migration', 19000)" in the webview for 20 seconds
    Then I collect JSON artifact "legacy-migration-reopened-chart" from webview expression "window.__e2e.workbench.assertMigratedResultChart('query_legacy_migration', 'chart_legacy_migration', 3)"
