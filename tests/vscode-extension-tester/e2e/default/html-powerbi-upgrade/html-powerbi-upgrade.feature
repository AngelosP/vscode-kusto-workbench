Feature: HTML dashboard Power BI upgrade notice
  ...
  Scenario: Restore-time compatibility scan only warns Power BI-relevant sections
    ...
    When I evaluate "(() => { const uploadButton = id => document.getElementById(id)?.shadowRoot?.querySelector('button[aria-label=\'Publish to Power BI service\']'); const captureClick = id => { const messages = []; window.__e2eCaptureHostMessage = message => { messages.push(message); return false; }; try { const button = uploadButton(id); if (!button) throw new Error(id + ' upload button missing'); button.click(); } finally { delete window.__e2eCaptureHostMessage; } return messages; }; const pureMessages = captureClick('html_pure_js'); if (pureMessages.length !== 1) throw new Error('Expected one pure JS upload message, got ' + pureMessages.length); const pure = pureMessages[0]; if (pure.type !== 'showPowerBiPublishHelp') throw new Error('Pure JS upload should ask for publish help: ' + JSON.stringify(pure)); if (pure.sectionId !== 'html_pure_js' || pure.sectionName !== 'Manual JavaScript dashboard') throw new Error('Pure JS upload targeted wrong section: ' + JSON.stringify(pure)); if (!Array.isArray(pure.reasons) || !pure.reasons.some(reason => String(reason).includes('Missing application/kw-provenance')) || !pure.reasons.some(reason => String(reason).includes('No query-backed data sources'))) throw new Error('Pure JS upload missing actionable reasons: ' + JSON.stringify(pure.reasons)); const heatMessages = captureClick('html_invalid_heatmap'); if (heatMessages.length !== 1) throw new Error('Expected one heatmap upload message, got ' + heatMessages.length); const heat = heatMessages[0]; if (heat.type !== 'showPowerBiUnsupportedVisualHelp') throw new Error('Heatmap upload should show unsupported visual help: ' + JSON.stringify(heat)); if (!String(heat.message || '').includes('does not support heatmap visuals yet')) throw new Error('Heatmap upload missing unsupported message: ' + JSON.stringify(heat)); if (!String(heat.message || '').includes('Ask for support for this chart type')) throw new Error('Heatmap upload missing support request: ' + JSON.stringify(heat)); if (String(heat.message || '').includes('heat (heatmap)')) throw new Error('Heatmap upload leaked raw analyzer reason: ' + heat.message); return 'enabled upload click validation messages verified'; })()" in the webview
