Feature: HTML dashboard Power BI upgrade notice
  ...
  Scenario: Restore-time compatibility scan only warns Power BI-relevant sections
    ...
    When I evaluate "(() => { const get = id => document.getElementById(id); const notice = id => get(id)?.shadowRoot?.querySelector('.power-bi-upgrade-notice'); const text = id => notice(id)?.textContent || ''; const expectNotice = (id, expected) => { const node = notice(id); if (!node) throw new Error(id + ' should show upgrade notice'); if (!text(id).includes(expected)) throw new Error(id + ' notice missing ' + expected + ': ' + text(id)); }; const expectNoNotice = id => { const node = notice(id); if (node) throw new Error(id + ' should not show notice: ' + node.textContent); }; expectNoNotice('html_valid_powerbi'); expectNoNotice('html_pure_js'); expectNotice('html_invalid_heatmap', 'heat (heatmap)'); expectNotice('html_invalid_target', 'top-table (table: target must be table or tbody inside table)'); expectNotice('html_published_old', 'Missing application/kw-provenance'); const pure = get('html_pure_js'); const pureRoot = pure?.shadowRoot; const wrapper = pureRoot?.querySelector('.header-tab-tooltip-wrapper'); if (!wrapper) throw new Error('disabled upload wrapper missing'); const title = wrapper.getAttribute('title') || ''; if (!title.includes('not set up for Power BI export yet')) throw new Error('disabled tooltip title missing expected text: ' + title); if (wrapper.getAttribute('tabindex') !== '0') throw new Error('disabled wrapper must be keyboard focusable'); if (wrapper.getAttribute('role') !== 'button') throw new Error('disabled wrapper must expose button role'); if (wrapper.getAttribute('aria-disabled') !== 'true') throw new Error('disabled wrapper must expose aria-disabled'); const describedBy = wrapper.getAttribute('aria-describedby'); const description = describedBy ? pureRoot?.getElementById(describedBy) : null; if (!description?.textContent?.includes('Ask the Kusto Workbench agent')) throw new Error('disabled upload sr-only explanation missing'); const disabledButton = wrapper.querySelector('button.header-tab'); if (!disabledButton?.disabled) throw new Error('pure JS upload button should be disabled'); const invalidButton = get('html_invalid_heatmap')?.shadowRoot?.querySelector('.header-tab-tooltip-wrapper button.header-tab'); if (invalidButton?.disabled) throw new Error('invalid Power BI section has bindings, so upload should be enabled'); return 'restore-time gate and disabled upload affordance verified'; })()" in the webview
    Then I take a screenshot "01-powerbi-relevance-gates"

    When I evaluate "(() => { const section = document.getElementById('html_invalid_heatmap'); const root = section?.shadowRoot; const messages = []; window.__e2eCaptureHostMessage = message => { messages.push(message); return false; }; try { const update = root?.querySelector('.power-bi-upgrade-primary'); if (!update) throw new Error('Update button missing'); update.click(); } finally { delete window.__e2eCaptureHostMessage; } if (messages.length !== 1) throw new Error('Expected exactly one upgrade message, got ' + messages.length); const msg = messages[0]; if (msg.sectionId !== 'html_invalid_heatmap') throw new Error('Unexpected sectionId: ' + msg.sectionId); if (msg.sectionName !== 'Unsupported heatmap dashboard') throw new Error('Unexpected sectionName: ' + msg.sectionName); if (!Array.isArray(msg.reasons) || !msg.reasons.includes('heat (heatmap)')) throw new Error('Missing heatmap reason: ' + JSON.stringify(msg.reasons)); return 'Update posted requestHtmlDashboardUpgradeWithCopilot'; })()" in the webview
  ...
  Scenario: Notice close and persistent dismissal respect the current issue signature
    ...
    And I evaluate "window.__e2e.workbench.clearSections()" in the webview
    And I wait 1 second
    When I evaluate "(() => { const html = '<script type=\'application/kw-provenance\'>' + JSON.stringify({ version: 1, model: { fact: { sectionId: 'query_fact', sectionName: 'Fact Events' } }, bindings: { heat: { display: { type: 'heatmap', xColumn: 'Day', valueColumn: 'Requests' } } } }) + '</script><div data-kw-bind=\'heat\'></div>'; window.addHtmlBox({ id: 'html_dismissal_case', name: 'Dismissal dashboard', code: html, mode: 'preview', expanded: true, previewHeightPx: 160 }); return 'created dismissal section'; })()" in the webview
    And I wait 3 seconds
    When I execute command "workbench.action.files.save"
    And I wait 3 seconds
    When I execute command "workbench.action.closeAllEditors"
    ...
    And I wait 1 second
    When I evaluate "(() => { const text = document.getElementById('html_dismissal_case')?.shadowRoot?.querySelector('.power-bi-upgrade-notice')?.textContent || ''; if (!text.includes('top-table (table: target must be table or tbody inside table)')) throw new Error('new signature should show new notice, got: ' + text); return 'new signature reappears'; })()" in the webview
  ...
  Scenario: Editing a Power BI-enabled HTML section recomputes upgrade status without reload
    ...
    And I wait 1 second
    When I evaluate "(() => { const section = document.getElementById('html_edit_recheck'); if (section?.shadowRoot?.querySelector('.power-bi-upgrade-notice')) throw new Error('valid section should start without notice'); const invalid = '<script type=\'application/kw-provenance\'>' + JSON.stringify({ version: 1, model: { fact: { sectionId: 'query_fact', sectionName: 'Fact Events' } }, bindings: { heat: { display: { type: 'heatmap', xColumn: 'Day', valueColumn: 'Requests' } } } }) + '</script><div data-kw-bind=\'heat\'></div>'; section.setCode(invalid); return 'introduced unsupported binding'; })()" in the webview
    And I wait 1 second
    When I evaluate "(() => { const text = document.getElementById('html_edit_recheck')?.shadowRoot?.querySelector('.power-bi-upgrade-notice')?.textContent || ''; if (!text.includes('heat (heatmap)')) throw new Error('new unsupported binding should show notice, got: ' + text); const valid = '<script type=\'application/kw-provenance\'>' + JSON.stringify({ version: 1, model: { fact: { sectionId: 'query_fact', sectionName: 'Fact Events' } }, bindings: { total: { display: { type: 'scalar', agg: 'COUNT' } } } }) + '</script><span data-kw-bind=\'total\'>0</span>'; document.getElementById('html_edit_recheck').setCode(valid); return 'restored valid binding'; })()" in the webview
    And I wait 1 second
