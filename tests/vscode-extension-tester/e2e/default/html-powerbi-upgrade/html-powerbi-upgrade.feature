Feature: HTML dashboard Power BI upgrade notice
  ...
  Scenario: Restore-time compatibility scan only warns Power BI-relevant sections
    ...
    When I evaluate "(() => { const query = document.getElementById('query_partial_fact'); if (!query) throw new Error('query_partial_fact missing'); query.serialize = () => ({ clusterUrl: 'https://cluster.example', database: 'Samples', query: 'print total=1' }); window.postMessage({ type: 'queryResult', boxId: 'query_partial_fact', result: { columns: [{ name: 'total', type: 'long' }], rows: [[1]], metadata: {} } }, '*'); return 'seeded partial publish query result'; })()" in the webview
    And I wait 1 second
