Feature: Kusto Copilot clarification handoff
  ...
  Scenario: Captured file identity follows a renamed target while the focused file stays untouched
    ...
    And I wait for "body[data-kusto-e2e-ready='true']" in the webview "clarification-a.kqlx" for 20 seconds
    ...
    And I execute command "workbench.action.keepEditor"
    ...
    And I wait for "body[data-kusto-e2e-ready='true']" in the webview "clarification-b.kqlx" for 20 seconds
    And I wait for "kw-query-section" in the webview for 20 seconds
    ...
    Then I collect JSON artifact "08-target-a-delegation-started" from extension host expression "(async () => { const deadline = Date.now() + 10000; while (Date.now() < deadline) { const providers = await vscode.commands.executeCommand('kustoWorkbench.test.getCopilotDevelopmentModelSnapshot'); const provider = providers.find(candidate => String(candidate.documentUri || '').replace(/\\/g, '/').endsWith('/clarification-a.kqlx')); if (provider?.model?.consumedResponses === 1 && provider.model.requests?.length === 1) return { documentUri: provider.documentUri, consumedResponses: provider.model.consumedResponses, requestCount: provider.model.requests.length, activeTab: vscode.window.tabGroups.activeTabGroup.activeTab?.label || '' }; await new Promise(resolve => setTimeout(resolve, 50)); } throw new Error('Target A delegation did not enter the scripted model while B was focused'); })()"
    Then I collect JSON artifact "08-focused-b-during-delegation" from extension host expression "(() => { const extension = vscode.extensions.all.find(candidate => candidate.packageJSON?.name === 'vscode-kusto-workbench'); const expected = vscode.Uri.joinPath(extension.extensionUri, 'tests', 'vscode-extension-tester', 'runs', 'default', 'kusto-copilot-clarification-handoff', 'clarification-b.kqlx').toString(); const activeUri = vscode.window.tabGroups.activeTabGroup.activeTab?.input?.uri?.toString() || ''; if (activeUri !== expected) throw new Error('Delegation moved focus away from file B: ' + activeUri); return { activeUri, expected }; })()"
    ...
    Then I collect JSON artifact "08-focused-b-after-rename" from extension host expression "(() => { const extension = vscode.extensions.all.find(candidate => candidate.packageJSON?.name === 'vscode-kusto-workbench'); const expected = vscode.Uri.joinPath(extension.extensionUri, 'tests', 'vscode-extension-tester', 'runs', 'default', 'kusto-copilot-clarification-handoff', 'clarification-b.kqlx').toString(); const activeUri = vscode.window.tabGroups.activeTabGroup.activeTab?.input?.uri?.toString() || ''; if (activeUri !== expected) throw new Error('Rename moved focus away from file B: ' + activeUri); return { activeUri, expected }; })()"
    ...
    Then I collect JSON artifact "08-focused-b-before-inspection" from extension host expression "(() => { const extension = vscode.extensions.all.find(candidate => candidate.packageJSON?.name === 'vscode-kusto-workbench'); const expected = vscode.Uri.joinPath(extension.extensionUri, 'tests', 'vscode-extension-tester', 'runs', 'default', 'kusto-copilot-clarification-handoff', 'clarification-b.kqlx').toString(); const activeUri = vscode.window.tabGroups.activeTabGroup.activeTab?.input?.uri?.toString() || ''; if (activeUri !== expected) throw new Error('Clarification settlement moved focus away from file B: ' + activeUri); return { activeUri, expected }; })()"
