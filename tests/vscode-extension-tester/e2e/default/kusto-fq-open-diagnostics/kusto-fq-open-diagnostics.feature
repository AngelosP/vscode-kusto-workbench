Feature: Fully qualified Kusto references prepare without an editor click
  ...
  Scenario: Restored KQLX sections keep supplemental schemas model-scoped
    ...
    When I evaluate "window.__e2e.kusto.waitForSupplementalState(1, 'waiting-primary', 15000)" in the webview for 20 seconds
    When I evaluate "(() => { const first = document.getElementById('query_supplemental_first'); const firstId = first?.boxId || first?.id; if (window.activeQueryEditorBoxId !== firstId) throw new Error('Inactive supplemental loading stole focus'); return 'inactive supplemental remained waiting-primary without focus'; })()" in the webview
    When I evaluate "(() => { const second = document.getElementById('query_supplemental_second'); if (!second) throw new Error('Inactive second section missing'); if (second.dataset.testPreparationState !== 'idle' || second.dataset.testPreparationStage !== 'idle' || second.getAttribute('aria-busy') !== 'false') throw new Error('Inactive second section exposed active preparation: ' + JSON.stringify({ state: second.dataset.testPreparationState, stage: second.dataset.testPreparationStage, ariaBusy: second.getAttribute('aria-busy') })); return 'inactive second section stayed idle and non-busy while supplemental state waited for primary'; })()" in the webview
