Feature: SQL section lifecycle - add, rename, collapse, expand, remove
  ...
  Scenario: Add, rename, collapse, expand, and remove SQL section without a SQL connection
    ...
    When I evaluate "(() => { const el = document.querySelector('kw-sql-section'); if (!el) throw new Error('No SQL section found'); const shell = el.shadowRoot?.querySelector('kw-section-shell'); if (!shell) throw new Error('No SQL section shell found'); const nameInput = shell.shadowRoot?.querySelector('input.query-name'); if (!(nameInput instanceof HTMLInputElement)) throw new Error('No .query-name input found'); nameInput.focus(); nameInput.value = 'My SQL Test'; nameInput.dispatchEvent(new InputEvent('input', { bubbles: true, inputType: 'insertText', data: 'My SQL Test' })); nameInput.dispatchEvent(new Event('change', { bubbles: true })); return 'renamed SQL section through .query-name'; })()" in the webview
    And I wait 1 second
