Feature: Imported CSV save application handler
  ...
  Scenario: Imported URL CSV writes exact UTF-8 bytes through the host handler
    When I delete file "${TEMP}\kusto-workbench-imported-hst4.csv"
    When I execute command "kustoWorkbench.test.setNextCsvSaveTarget" with args '["kusto-workbench-imported-hst4.csv"]'
    ...
    Then I collect JSON artifact "hst4-imported-csv-bytes" from extension host expression "(async () => { const base = String(process.env.TEMP || process.env.TMP || ''); const uri = vscode.Uri.joinPath(vscode.Uri.file(base), 'kusto-workbench-imported-hst4.csv'); const newline = String.fromCharCode(10); const expected = 'Name,City' + newline + 'Jos\u00e9,\u6771\u4eac' + newline + 'Zo\u00eb,M\u00fcnchen'; const deadline = Date.now() + 10000; let lastError = ''; while (Date.now() < deadline) { try { const bytes = await vscode.workspace.fs.readFile(uri); const text = new TextDecoder().decode(bytes); if (text !== expected) throw new Error('Imported CSV bytes differ: ' + JSON.stringify({ expected, actual: text })); return { text, byteLength: bytes.byteLength, uri: uri.toString() }; } catch (error) { lastError = String(error?.message || error); await new Promise(resolve => setTimeout(resolve, 100)); } } throw new Error('Imported CSV write did not settle: ' + lastError); })()"
