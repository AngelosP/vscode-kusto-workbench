Feature: SQL favorites — star toggle, mode switch, dropdown
  ...
  Scenario: Add favorite, switch to favorites mode, select, remove
    ...
    When I evaluate "(() => { const el = document.querySelector('kw-sql-section'); const connectionId = el.getSqlConnectionId(); const database = el.getDatabase(); const matching = (window.sqlFavorites || []).filter(f => f.connectionId === connectionId && String(f.database || '').toLowerCase() === String(database || '').toLowerCase()); if (!matching.length) return 'no existing matching favorite'; const star = el.shadowRoot?.querySelector('.favorite-btn'); if (!star) throw new Error('Favorite star button not found for pre-cleanup'); star.click(); return 'removed existing matching favorite'; })()" in the webview
    And I wait 2 seconds
    When I evaluate "(() => { const el = document.querySelector('kw-sql-section'); const connectionId = el.getSqlConnectionId(); const database = el.getDatabase(); const remaining = (window.sqlFavorites || []).filter(f => f.connectionId === connectionId && String(f.database || '').toLowerCase() === String(database || '').toLowerCase()); if (remaining.length) throw new Error('Pre-cleanup left ' + remaining.length + ' matching favorites'); return 'pre-cleanup verified'; })()" in the webview
    ...
    When I evaluate "(() => { const el = document.querySelector('kw-sql-section'); const star = el.shadowRoot?.querySelector('.favorite-btn'); if (!star) throw new Error('Favorite star button not found'); star.click(); return 'clicked favorite star for ' + el.getDatabase(); })()" in the webview
    And I wait 1 second
