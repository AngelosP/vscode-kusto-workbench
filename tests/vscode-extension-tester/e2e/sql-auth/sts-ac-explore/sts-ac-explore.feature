Feature: Comprehensive T-SQL autocomplete exploration
  ...
  Scenario: Explore all interesting T-SQL autocomplete contexts
    ...
    # EXPECTED: column names from SalesLT.Product
    # ══════════════════════════════════════════════════════════════════════
    When I evaluate "window.__e2e.suggest.sql.setTextAt(`SELECT Color, COUNT(*)\nFROM SalesLT.Product\nGROUP BY `, 3, 10)" in the webview
    And I wait 1 second
    When I evaluate "window.__e2e.suggest.sql.trigger()" in the webview
    And I wait 5 seconds
    ...
    # EXPECTED: column names from SalesLT.Product
    # ══════════════════════════════════════════════════════════════════════
    When I evaluate "window.__e2e.suggest.sql.setTextAt('SELECT COUNT() FROM SalesLT.Product', 1, 14)" in the webview
    And I wait 1 second
    When I evaluate "window.__e2e.suggest.sql.trigger()" in the webview
    And I wait 5 seconds
    ...
    # EXPECTED: column names from SalesLT.Product
    # ══════════════════════════════════════════════════════════════════════
    When I evaluate "window.__e2e.suggest.sql.setTextAt('UPDATE SalesLT.Product SET ', 1, 28)" in the webview
    And I wait 1 second
    When I evaluate "window.__e2e.suggest.sql.trigger()" in the webview
    And I wait 5 seconds
    ...
    # EXPECTED: column names from SalesLT.Product
    # ══════════════════════════════════════════════════════════════════════
    When I evaluate "window.__e2e.suggest.sql.setTextAt('INSERT INTO SalesLT.Product ()', 1, 30)" in the webview
    And I wait 1 second
    When I evaluate "window.__e2e.suggest.sql.trigger()" in the webview
    And I wait 5 seconds
    ...
    # ══════════════════════════════════════════════════════════════════════
    When I evaluate "window.__e2e.suggest.sql.setTextAt(`WITH cte AS (\n  SELECT  FROM SalesLT.Product\n)\nSELECT * FROM cte`, 2, 10)" in the webview
    And I wait 1 second
    When I evaluate "window.__e2e.suggest.sql.trigger()" in the webview
    And I wait 5 seconds
    ...
    # ══════════════════════════════════════════════════════════════════════
    When I evaluate "window.__e2e.suggest.sql.setTextAt('SELECT * FROM Prod', 1, 19)" in the webview
    And I wait 1 second
    When I evaluate "window.__e2e.suggest.sql.trigger()" in the webview
    And I wait 5 seconds
