...
| Isolated session reset -> exact close drain -> accepted initial projection | Only the current live panel's host-accepted activation releases the test opener; a rejected first generation does not qualify | Include `firstLaunchIntegration.test.ts` held close/write/initialization cases and `kqlxMarkdownOwnership.test.ts`, `initial projection initialization waiter handles ...`; composed E2E includes `kusto-results-visibility-layout`, `share-result-artifacts`, and `section-lifecycle`; ordinary production opens remain unchanged |
