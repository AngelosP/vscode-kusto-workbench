...
import { getWorkbenchTabInputUris, normalizeWorkbenchUriKey } from './workbenchFileTypes';
	...
	public async resolveCustomTextEditor(
		...
		const startupGateway = new MainWebviewStartupGateway<IncomingWebviewMessage>({
			...
			admitInbound: input => {
				...
				if (isCompatibilityPersistenceWebviewMessageType(input)) {
					const parsed = parseCompatibilityPersistenceWebviewMessage(input, 'kql');
					if (!parsed.ok || parsed.value.viewSessionId !== viewSessionId) return undefined;
					if (outerDisposed && !closeCoordinator.allowRetiredInbound(parsed.value)) return undefined;
					return parsed.value;
				}
			...
			prepareOutbound: message => {
				if (!isCompatibilityPersistenceHostMessageType(message)) return message;
				const parsed = stampCompatibilityPersistenceHostMessage(viewSessionId, message);
				if (!parsed.ok) {
					getWorkbenchLogger().warn(`[kusto] Rejected invalid compatibility persistence host message: ${parsed.error}`);
					return undefined;
				}
				return parsed.value;
			},
		...
		const requireAvailableFinalPersist = async (reason: string): Promise<void> => {
			...
			requireAvailableCompatSidecarFinalPersist(result, 'KQL');
		};
		...
		const closeFinalization: CompatSidecarCloseFinalization = {
			...
			inspectToolDirty: async () => {
				...
				if (projectionCoordinator.sourceRollbackFailed || activeSourceMutations > 0) {
					throw new Error('The KQL compatibility editor is still settling a source update.');
				}
			...
			saveToolChanges: async () => {
				...
				try {
					...
					if (document.isDirty && !await document.save()) {
						throw new Error('VS Code rejected the KQL source save.');
					}
					await persistCompanionAfterPrimarySave(document, assertNoDirtyOpenSidecarAlias);
					await sidecarSession.waitForPersists();
					await assertNoDirtyOpenSidecarAlias();
					if (document.isDirty || sidecarSession.isDirty) {
						throw new Error('The KQL source or companion metadata remained dirty after saving.');
					}
				} finally {
		...
		handleIncomingWebviewMessage = async (message: IncomingWebviewMessage) => {
			...
			switch (message.type) {
				...
				case 'requestUpgradeToKqlx': {
					...
					try {
						void startupGateway.postMessage({ type: 'enabledKqlxSidecar', addKind: normalizedAddKind });
					} catch {
						// ignore
					}
					return;
