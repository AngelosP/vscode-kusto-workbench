...
export class SqlCompatEditorProvider implements vscode.CustomTextEditorProvider {
	...
	public async resolveCustomTextEditor(
		...
		const startupGateway = new MainWebviewStartupGateway<IncomingWebviewMessage>({
			...
			admitInbound: input => {
				...
				if (isCompatibilityPersistenceWebviewMessageType(input)) {
					const parsed = parseCompatibilityPersistenceWebviewMessage(input, 'sql');
		...
		type SqlProjectionRequest = CompatSidecarProjectionRequest & Readonly<{
		...
		const projectionCoordinator = this.projectionCoordinatorFactory({
			...
			postProjection: async projection => {
				...
				const htmlPowerBiCompatibilityCheckEnabled = vscode.workspace.getConfiguration('kustoWorkbench').get<boolean>('html.powerBiCompatibilityCheck.enabled', true);
				const sqlText = projection.sourceText;
		...
		const postDocument = (options?: SqlProjectionRequest): Promise<boolean> => {
		...
		handleIncomingWebviewMessage = async (message: IncomingWebviewMessage) => {
			...
			switch (message.type) {
				...
				case 'persistDocument': {
					...
					try {
						if (!sidecarFile) {
							assertCompatPrimaryIdentity(rawState, 'sql', PLAIN_SQL_PRIMARY_SECTION_ID);
						...
						SqlCompatEditorProvider.buildSidecarFileForCompat(document.uri, rawState, sidecarFile);
					} catch (error) {
						if (flushRequestId) {
					...
					const run = sidecarSession.queuePersist(incomingEditRevision, async persistIsCurrent => {
						...
						try {
							...
							SqlCompatEditorProvider.buildSidecarFileForCompat(document.uri, incomingRawState, sidecarFile);
						} catch (error) {
