...
export const styles = css`
	*,*::before,*::after{box-sizing:border-box}
	...
	.ppd-data-mode-copy{display:grid;grid-template-columns:1fr 1fr;gap:8px}
	.ppd-mode-copy{min-width:0;padding:7px 8px;border-radius:4px;border:1px solid var(--vscode-panel-border);background:rgba(255,255,255,.02);color:var(--vscode-descriptionForeground);font-size:11px;line-height:1.35;font-family:inherit;text-align:left;cursor:pointer}
	.ppd-mode-copy:hover:not(:disabled):not(.is-active){background:var(--vscode-list-hoverBackground)}
	.ppd-mode-copy:focus-visible{outline:1px solid var(--vscode-focusBorder);outline-offset:2px}
	.ppd-mode-copy:disabled{opacity:.55;cursor:default}
