...
import {
	tryParseFiniteNumber as _tryParseFiniteNumber,
	tryParseDate as _tryParseDate,
	formatDate as _formatDate,
	getRawCellValue,
	tokenizeExpr as _tokenizeExpr,
...
export function __kustoTryParseFiniteNumber( v: any) {
	try {
		return _tryParseFiniteNumber(v);
	} catch {
		return null;
	}
}
...
export function __kustoFormatDate( d: any, fmt: any) {
	try { return _formatDate(d, fmt); } catch { return null; }
}
...
try {
	...
	window.__kustoOnResultsVisibilityToggled = (boxId: any) => {
		...
		try {
			...
			let visible = true;
			try {
				visible = !(pState.resultsVisibleByBoxId && pState.resultsVisibleByBoxId[id] === false);
			} catch (e) { console.error('[kusto]', e); }
			if (!visible) {
