// Search control module — converted from legacy/searchControl.js
...
function __kustoNormalizeSearchMode(mode: string | undefined): string {
	const m = String(mode || '').toLowerCase();
	return (m === SEARCH_MODE_REGEX) ? SEARCH_MODE_REGEX : SEARCH_MODE_WILDCARD;
}

function __kustoUpdateSearchModeToggle(btn: HTMLElement | null, mode: string): void {
	if (!btn) return;
...
function __kustoGetSearchIconSvg(): string {
...
function __kustoGetWildcardIconSvg(): string {
...
function __kustoGetRegexIconSvg(): string {
...
function __kustoGetChevronUpSvg(): string {
...
function __kustoGetChevronDownSvg(): string {
...
function __kustoUpdateSearchStatus(statusEl: HTMLElement | null, matchCount: number, currentMatchIndex: number, hasError: boolean, errorMsg?: string): void {
	if (!statusEl) return;
	if (hasError) {
...
function __kustoSetSearchNavEnabled(prevBtn: HTMLButtonElement | null, nextBtn: HTMLButtonElement | null, enabled: boolean, matchCount: number): void {
...
function __kustoTrySyncSearchAffordancesVerticalPositionToInput(input: HTMLElement | null, elements: Array<HTMLElement | null>): void {
...
function __kustoCreateSearchControl(hostEl: HTMLElement | null, options?: SearchControlOptions) {
	if (!hostEl) return null;
	const opts: Partial<SearchControlOptions> = options && typeof options === 'object' ? options : {};
...
function __kustoGetSearchControlState(inputId: string, modeId: string): { query: string; mode: string } {
	try {
		const input = document.getElementById(inputId) as HTMLInputElement | null;
		const modeEl = document.getElementById(modeId) as HTMLElement | null;
		const modeVal = modeEl ? ((modeEl as HTMLElement).dataset.mode || (modeEl as HTMLInputElement).value || SEARCH_MODE_WILDCARD) : SEARCH_MODE_WILDCARD;
		return {
...
function __kustoTryBuildSearchRegex(query: string, mode: string): { regex: RegExp | null; error: string | null; mode: string } {
...
function __kustoRegexTest(regex: RegExp | null, text: string): boolean {
...
function __kustoCountRegexMatches(regex: RegExp | null, text: string, maxMatches?: number): number {
	if (!regex) return 0;
	const s = String(text || '');
...
function __kustoHighlightPlainTextToHtml(text: string, regex: RegExp | null, options?: HighlightOptions): { html: string; count: number } {
	const s = String(text || '');
...
function __kustoHighlightElementTextNodes(rootEl: HTMLElement | null, regex: RegExp | null, highlightClass?: string): number {
	if (!rootEl) return 0;
	if (!regex) return 0;
...
// ── Window bridge ──
_win.__kustoCreateSearchControl = __kustoCreateSearchControl;
_win.__kustoGetSearchControlState = __kustoGetSearchControlState;
_win.__kustoTryBuildSearchRegex = __kustoTryBuildSearchRegex;
_win.__kustoRegexTest = __kustoRegexTest;
_win.__kustoCountRegexMatches = __kustoCountRegexMatches;
_win.__kustoHighlightPlainTextToHtml = __kustoHighlightPlainTextToHtml;
_win.__kustoHighlightElementTextNodes = __kustoHighlightElementTextNodes;
_win.__kustoUpdateSearchModeToggle = __kustoUpdateSearchModeToggle;
_win.__kustoUpdateSearchStatus = __kustoUpdateSearchStatus;
_win.__kustoSetSearchNavEnabled = __kustoSetSearchNavEnabled;
