// Copilot chat module — converted from legacy/copilotQueryBoxes.js
...
function __kustoIsNearBottom(host: any) {
	try {
 ...
 */
function __kustoCopilotAutoScrollToBottom(host: any) {
	try {
		if (!host) return;
...
let __kustoCurrentVisibleTooltip: HTMLElement | null = null;
let __kustoCurrentHideTimeout: ReturnType<typeof setTimeout> | null = null;

// Helper to set up hover tooltip behavior with fixed positioning
function __kustoSetupToolTooltip(wrapper: any, tooltip: any) {
...
function __kustoCopilotLogoHtml() {
	const uri = (() => {
		try {
			return ((_win.__kustoQueryEditorConfig as any) && (_win.__kustoQueryEditorConfig as any).copilotLogoUri)
				? String((_win.__kustoQueryEditorConfig as any).copilotLogoUri)
				: '';
		} catch {
			return '';
		}
...
function __kustoCopilotChatHtml(boxId: any) {
	const modelDropdown = (() => {
		try {
			if ((_win.__kustoDropdown as any) && typeof (_win.__kustoDropdown as any).renderMenuDropdownHtml === 'function') {
...
function __kustoSafeToolDomId(name: any) {
	return String(name || '').replace(/[^a-zA-Z0-9_\-]/g, '_');
}

function __kustoGetDefaultEnabledTools(tools: any) {
...
function __kustoSetCopilotToolsOptions(boxId: any, tools: any) {
	...
	if (!stateByBox[id].userModified) {
		...
		try {
			const known = new Set((stateByBox[id].tools || []).map((t: any) => String((t && t.name) || '')));
...
function __kustoGetEnabledToolsForNextMessage(boxId: any) {
	...
	const known = new Set(tools.map((t: any) => String((t && t.name) || '')));
	return enabled.map(String).filter((n: any) => n && known.has(String(n)));
}

function __kustoResetEnabledToolsForNextMessage(boxId: any) {
	const id = String(boxId || '').trim();
	if (!id) return;
	const stateByBox = __kustoEnsureCopilotToolSelectionState();
	const st = stateByBox[id] || {};
...
function __kustoRenderCopilotToolsPanel(boxId: any) {
	const id = String(boxId || '').trim();
	if (!id) return;
	const listHost = document.getElementById(id + '_copilot_tools_list') as any;
	const toolsBtn = document.getElementById(id + '_copilot_tools_btn') as any;
	if (!listHost) return;
	const stateByBox = __kustoEnsureCopilotToolSelectionState();
	const st = stateByBox[id] || {};
	const tools = Array.isArray(st.tools) ? st.tools : [];
...
function __kustoAppendChatMessage(boxId: any, role: any, text: any, detail: any = '') {
	try {
		...
		const safeRole = (roleMap as any)[role] || 'assistant';
...
function __kustoSetCopilotChatRunning(boxId: any, running: any, statusText: any = '') {
	...
	try {
		...
		const toolsBtn = document.getElementById(boxId + '_copilot_tools_btn') as any;
...
function __kustoApplyModelOptions(boxId: any, models: any, selectedModelId: any) {
	try {
		const sel = document.getElementById(boxId + '_copilot_model') as any;
		if (!sel) return;
		sel.innerHTML = '';
...
function __kustoInstallCopilotChat(boxId: any) {
	try {
		...
		if (!wrapper) return;
		const editorWrapper = wrapper.closest ? wrapper.closest('.query-editor-wrapper') as HTMLElement | null : null;
		...
		try {
			const inputEl = document.getElementById(boxId + '_copilot_input') as any;
			if (inputEl) {
...
function __kustoSetCopilotChatVisible(boxId: any, visible: any) {
	...
	if (next) {
		try {
			...
			const editorWrapper = wrapper && wrapper.closest ? wrapper.closest('.query-editor-wrapper') as HTMLElement | null : null;
			if (editorWrapper) {
				...
				if (currentHeight < minHeight) {
					...
					try {
						if (!(_win.__kustoManualQueryEditorHeightPxByBoxId as any) || typeof (_win.__kustoManualQueryEditorHeightPxByBoxId as any) !== 'object') {
...
window.__kustoCopilotWriteQuerySend = function __kustoCopilotWriteQuerySend(boxId: any) {
	...
	const inputEl = document.getElementById(id + '_copilot_input') as any;
	...
	try {
		...
		(_win.vscode as any).postMessage({
			...
			enabledTools,
			queryMode: typeof (_win.getRunMode) === 'function' ? (_win.getRunMode as any)(id) : 'take100'
		});
...
let __kustoActiveCopilotToolsPanel: string | null = null;

function __kustoCopilotCloseToolsPanel(boxId: any) {
	const id = String(boxId || '').trim();
	if (!id) return;
	const panel = document.getElementById(id + '_copilot_tools_panel');
...
document.addEventListener('click', function (e) {
	...
	if (panel.contains(e.target as Node) || (btn && btn.contains(e.target as Node))) return;
				...
				if (typeof window.addQueryBox === 'function') {
					...
					if (newBoxId) {
						setTimeout(() => {
							...
							if (storedResult && typeof (_win.displayResultForBox) === 'function') {
								(_win.displayResultForBox as any)(storedResult, newBoxId, { label: 'Results', showExecutionTime: true });
							}
