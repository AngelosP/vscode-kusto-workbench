...
* Result tables can show complete complex-value text beside each existing `View` action, configure that column's existing width cap, and highlight table-search matches inside the preview.
...
* Bugs
    * Some models would occasionally show tool call errors in versions v1.121.0+ of VS Code (more edge cases fixed)
...
* Finalized UX of 'Did you know?' feature that lets users know of product functionality in piece-meal fashion.
* Bugs
...
* Bugs
    ...
    * Icons on in the Activity Bar were not the correct ones in a couple of places.
    * Some non-user settings were exposed in the extension's settings, like debugging MD sections, so removed those.
...
* Insert section gesture! Keep your mouse over the bottom of the section like you would to resize it and you'll see it.
...
* Bugs:
    * Table headers became transparent after changes to the VS Code themes ... take #2 :(
