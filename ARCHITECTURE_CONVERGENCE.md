...
The comparison was refreshed against the current working tree after HST-21 KQL language request application-handler closure. It includes the full implemented feature set described by the README, package contributions, persisted formats, architecture documentation, browser extension, and test suites.
...
| 1 | `HST` | Host application composition; retire `QueryEditorProvider` as an application shell | 4/4/5/4/3/5 | 46 | HST-1 through HST-21 closed; HST-22 selected |
...
**Why next:** HST remains the highest eligible gap at 46 after rescoring. `saveSqlLastSelection` is the smallest remaining complete provider-owned application workflow: one response-free route, trimmed connection admission, two ordered application-state writes, no mutable map, and no lifecycle or transport terminal. Development-note, comparison-summary, and inline-completion candidates remain more coupled to request maps, document mutation, SQL owner/privacy admission, or Copilot execution.

**Boundary:** extract only `saveSqlLastSelection`, trimmed non-empty connection admission, ordered `sql.lastConnectionId` then optional `sql.lastDatabase` application-state writes, exact `undefined` versus empty-database behavior, exact rejection, and disposal into one injected `HostSqlLastSelectionApplicationHandler`. Append the constructor slot immediately after `kqlLanguageRequestApplication`.

**Preserved owners:** `SqlSectionSessionController` retains target transitions and message emission. `HostSqlConnectionOnboardingApplicationHandler` retains its independent post-add selection write. `QueryEditorProvider` retains revisioned policy/principal-aware `sqlConnectionsData` projection and reads the same application-state keys. `SqlEditorLifecycleCoordinator`, SQL connections/secrets, discovery, schema, execution, comparisons, persistence, Copilot, controls, response routing, and message shapes remain unchanged.

**Falsifiable hypothesis:** if one handler owns Query Editor SQL last-selection persistence, the real provider can reference-identically forward the exact request and adopt its settlement without directly writing application state.

**Cheapest discriminating check:** add `queryEditorProviderSqlLastSelectionHandler.test.ts` without importing the future module, append a structural fake immediately after the KQL language-request constructor slot, send an exact `{ type: 'saveSqlLastSelection', sqlConnectionId: ' sql-a ', database: '' }` object, and prove one reference-identical call, deferred settlement, exact rejection, and zero provider `globalState.update` effects. Record the intended zero-call failure with `--maxWorkers=1` before production edits.

**Exclusions:** no HST-1 through HST-21 changes; no SQL target/lifecycle, onboarding, connection projection, discovery, schema, execution, authentication, comparison, Copilot, persistence, protocol, browser, generic handler framework, or deferred ACT work.
