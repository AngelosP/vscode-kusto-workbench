...
The comparison was refreshed against the current working tree after HST-22 SQL last-selection application-handler closure. It includes the full implemented feature set described by the README, package contributions, persisted formats, architecture documentation, browser extension, and test suites.
...
| 1 | `HST` | Host application composition; retire `QueryEditorProvider` as an application shell | 4/4/5/4/3/5 | 46 | HST-1 through HST-22 closed; HST-23 selected |
...
**Why next:** HST remains the highest eligible gap at 46 after rescoring. Development-note mutation is the narrowest remaining complete provider-owned request-correlation workflow: one resolver map, one five-second deadline, one outbound mutation request, exact delivery-failure shaping, claimed `toolResponse` settlement, and disposal cleanup. Inline completion remains coupled to SQL owner/privacy admission and Copilot execution; comparison-summary state has broader comparison coupling and no active runtime waiter caller.

**Boundary:** extract only provider `webviewMutationResponseResolvers`, `updateDevelopmentNotes`, development-note request-ID generation, the five-second timeout, exact rejected/failed delivery results, matching `toolResponse` settlement, and panel-disposal settlement into one injected `HostDevelopmentNoteMutationApplicationHandler`. Append the constructor slot immediately after `sqlLastSelectionApplication`. `QueryEditorProvider` retains its `CopilotServiceHost.updateDevelopmentNotes()` method only as a thin delegate and must continue forwarding every unclaimed `toolResponse` to `toolOrchestrator`.

**Preserved owners:** `CopilotService` retains development-note add/supersede/remove semantics, note IDs/content/category shaping, tool-result copy, and conversation history. The webview retains `updateDevNotes` validation, compatibility/read-only admission, document mutation, persistence, and exact `toolResponse` construction. Agent tools retain their independent request/response path. Tool orchestration, other tool responses, document ownership, Kusto/SQL execution, Copilot model lifecycle, and message shapes remain unchanged.

**Falsifiable hypothesis:** if one handler owns only Copilot development-note request correlation, the real provider can delegate exact mutation requests and matching responses without retaining a resolver map while unrelated `toolResponse` traffic still reaches `toolOrchestrator` unchanged.

**Cheapest discriminating check:** add `queryEditorProviderDevelopmentNoteMutationHandler.test.ts` without importing the future module and append a structural fake immediately after the SQL last-selection constructor slot. Call `provider.updateDevelopmentNotes({ action: 'add', entry: { id: 'note_1' } })`, then send exact claimed `{ type: 'toolResponse', requestId: 'copilot_devnotes_test_1', result: { success: false }, error: 'Development notes require a companion metadata file.' }` and unclaimed `{ type: 'toolResponse', requestId: 'tool_other_1', result: { success: true } }` objects. Prove exact mutation delegation and settlement, reference-identical claimed-response offering, zero provider resolver-map effects, and unchanged unclaimed delivery to `toolOrchestrator`. Record the intended zero-call failure before production edits.

**Exclusions:** no HST-1 through HST-22 changes; no general tool-response broker, agent-tool migration, development-note semantics or webview mutation changes, document ownership, Kusto/SQL execution, Copilot model lifecycle, comparison-summary or inline-completion work, protocol/browser changes, generic handler framework, or deferred ACT work.
