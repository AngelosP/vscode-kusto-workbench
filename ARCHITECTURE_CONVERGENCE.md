...
The comparison was refreshed against the current working tree after HST-14 editing-preferences application-handler closure. It includes the full implemented feature set described by the README, package contributions, persisted formats, architecture documentation, browser extension, and test suites.
...
| 1 | `HST` | Host application composition; retire `QueryEditorProvider` as an application shell | 4/4/5/4/4/5 | 47 | HST-1 through HST-14 closed; HST-15 selected |
...
**Why next:** HST remains the highest eligible gap. `addConnectionsForClusters`, `promptImportConnectionsXml`, and `importConnectionsFromXml` form one bounded connection-intake workflow: short-name and physical-identity deduplication, normalization, sequential additions, native XML selection/read, import notifications, and provider-owned post-mutation connection refresh ordering. Favorites and single-connection onboarding/testing are broader and remain separate.

**Boundary:** extract only those three routes and their current intake orchestration into one injected `HostKustoConnectionIntakeApplicationHandler`. Keep `ConnectionManager` as the persistent connection owner. Keep `QueryEditorProvider`'s revisioned `connectionsData` snapshot/tail as an injected refresh effect. Keep webview XML parsing, existing message shapes, connection/auth lifecycle, database/schema work, and selection persistence unchanged.

**Falsifiable hypothesis:** if one connection-intake handler owns these three routes and refresh sequencing, `QueryEditorProvider` can forward each exact typed object and await claimed work without retaining intake decisions, while deduplication, picker/read behavior, notifications, connection persistence, and ordered connection projection remain unchanged.

**Cheapest discriminating check:** append one structural fake connection-intake handler to the real provider constructor, send all three exact message objects, and prove reference-identical forwarding in order while `ConnectionService` intake methods, provider refresh, and provider transport remain untouched. Before injection the fake receives zero calls and the provider executes the current branches directly.

**Exclusions:** no HST-1 through HST-14 changes; no favorites, `promptAddConnection`, `addConnection`, `testKustoConnection`, connection/auth lifecycle, database/schema behavior, selection persistence, webview XML parsing, generic connection framework, SQL, Copilot, tools, first launch, configuration, browser, document, or deferred ACT work.
