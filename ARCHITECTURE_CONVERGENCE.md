...
| 1 | `HST` | Host application composition; retire `QueryEditorProvider` as an application shell | 4/4/5/4/4/5 | 47 | HST-1 through HST-17 closed; HST-18 selected |
...
**Why next:** HST remains the highest eligible gap. `requestAddSqlFavorite` and `removeSqlFavorite` form one bounded profile workflow containing native naming UX, SQL connection label lookup, favorite validation/keying/upsert/removal, `sql.favorites` persistence, and `sqlFavoritesData` publication. The equivalent Connection Manager workflow already uses shared pure favorite mutations, while the Query Editor path still duplicates those decisions in `ConnectionService`.

**Boundary:** extract only the two provider routes and the complete SQL-favorites methods currently owned by `ConnectionService` into one injected `HostSqlFavoritesApplicationHandler`. Reuse the existing pure SQL favorite helpers from `connectionManagerFavorites.ts` where behavior is equivalent. Preserve exact prompt text/options/default name, cancellation, case-insensitive database keying, list order/replacement semantics, `sql.favorites` storage, originating `boxId` publication, contained publication failure logging, synchronous decline, and disposal settlement.

**Falsifiable hypothesis:** if one SQL favorites handler owns both routes, the real provider can reference-identically forward exact add/remove objects without invoking `ConnectionService`, native prompts, SQL connection lookup, global state, logging, or transport directly, while Query Editor and Connection Manager continue to derive the same favorite mutations from the shared pure helpers.

**Cheapest discriminating check:** append one structural fake SQL favorites handler to the real provider constructor, send exact `requestAddSqlFavorite` and `removeSqlFavorite` objects, and prove ordered reference-identical forwarding while legacy `ConnectionService` methods and every native/storage/transport effect remain untouched. Record the intended zero-call failure with `--maxWorkers=1` before production changes.

**Exclusions:** no HST-1 through HST-17 changes; no Kusto favorites, Connection Manager UI routes, SQL onboarding, connection listing/selection, databases/schemas, Leave No Trace filtering, STS, auth overrides, execution, Copilot, tools, browser, documents, generic favorites framework, or deferred ACT work.
