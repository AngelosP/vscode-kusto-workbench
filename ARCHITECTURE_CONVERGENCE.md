...
The comparison was refreshed against the current working tree after HST-18 SQL favorites application-handler closure. It includes the full implemented feature set described by the README, package contributions, persisted formats, architecture documentation, browser extension, and test suites.
...
| 1 | `HST` | Host application composition; retire `QueryEditorProvider` as an application shell | 4/4/5/4/4/5 | 47 | HST-1 through HST-18 closed; HST-19 selected |
...
**Why next:** HST remains the highest eligible gap. `requestAddFavorite`, `removeFavorite`, and `confirmRemoveFavorite` form one bounded Query Editor workflow containing naming/removal UX, account-partition-sensitive ownership, `kusto.favorites` migration and merge, cross-panel/listener publication, and exact result shaping. These decisions still live in `ConnectionService` beside unrelated selection and database-discovery behavior.

**Boundary:** extract only the three provider routes and the complete current Kusto-favorites methods/state from `ConnectionService` into one injected `HostKustoFavoritesApplicationHandler`. Reuse pure Kusto favorite helpers from `connectionManagerFavorites.ts` where behavior is equivalent. Preserve migration, active-principal filtering, account-partition revalidation, hidden-favorite merge, live-service/listener broadcast semantics, prompt/confirmation copy and defaults, add-only originating `boxId`, exact response/publication behavior, failure containment, synchronous decline, and disposal settlement.

**Falsifiable hypothesis:** if one Kusto favorites handler owns all three routes plus the existing cross-panel/listener publication boundary, the real provider can reference-identically forward exact add/remove/confirm objects without invoking `ConnectionService`, native prompts, global state, connection/account lookup, logging, or transport directly, while provider `connectionsData`, Connection Manager Kusto-favorites routes, extension listeners, and all principal-sensitive behavior remain unchanged.

**Cheapest discriminating check:** append one structural fake Kusto favorites handler to the real provider constructor, send exact `requestAddFavorite`, `removeFavorite`, and `confirmRemoveFavorite` objects, and prove ordered reference-identical forwarding while legacy `ConnectionService` methods and every prompt/storage/account/broadcast/transport effect remain untouched. Record the intended zero-call failure with `--maxWorkers=1` before production changes.

**Exclusions:** no HST-1 through HST-18 changes; no SQL favorites, Connection Manager routes, connection listing/selection, databases/schemas, onboarding/intake, Leave No Trace policy redesign, auth overrides, execution, Copilot, tools, browser, documents, generic favorites framework, or deferred ACT work.
