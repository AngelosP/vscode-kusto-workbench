...
The comparison was refreshed against the current working tree after HST-19 Kusto favorites application-handler closure. It includes the full implemented feature set described by the README, package contributions, persisted formats, architecture documentation, browser extension, and test suites.
...
| 1 | `HST` | Host application composition; retire `QueryEditorProvider` as an application shell | 4/4/5/4/3/5 | 46 | HST-1 through HST-19 closed; HST-20 selected |
...
**Why next:** HST remains the highest eligible gap at 46. `getSqlDatabases` and `refreshSqlDatabases` form the smallest remaining complete provider-owned state transition with privacy significance, exact lifecycle ownership, cache sequencing, and strong existing adversarial tests. Larger remaining provider workflows combine execution, comparison, persistence, or policy concerns and are less reversible.

**Boundary:** extract only `getSqlDatabases`, `refreshSqlDatabases`, their target-adoption step, `sendSqlDatabases`, and the discovery-specific SQL message-protection helpers into one injected `HostSqlDatabaseDiscoveryApplicationHandler`. Preserve passive-versus-refresh behavior, request identity/currentness, principal and Leave No Trace admission, protected one-shot discovery, cache ordering and fallback, sorting, loading/data/error terminals, logging, and disposal settlement.

**Preserved owners:** `SqlEditorLifecycleCoordinator` retains section identity, targets, generations, request tickets, and currentness. `SqlWorkbenchService` retains principal and Leave No Trace admission. `sqlDatabaseCache.ts` retains cross-window cache ownership/CAS; `SqlConnectionManager` and `SqlQueryClient` retain connection and physical discovery. The provider retains revisioned `sqlConnectionsData`, SQL favorites, schema, execution, STS, persistence, and comparisons. Webview emitters, response routing, and message shapes remain unchanged.

**Falsifiable hypothesis:** if one discovery handler owns both routes and the complete acquisition/publication sequence through narrow lifecycle, policy, cache, and transport capabilities, the real provider can reference-identically forward both exact objects without directly adopting targets, dispatching discovery, reading/writing cache state, publishing terminals, notifying, or logging.

**Cheapest discriminating check:** append a structural fake SQL database-discovery handler after the Kusto favorites constructor slot, send exact `getSqlDatabases` and `refreshSqlDatabases` objects, and prove ordered reference-identical forwarding with awaited settlement while provider discovery, target, policy, cache, transport, notification, and logging effects remain untouched. Record the intended zero-call failure with `--maxWorkers=1` before production changes.

**Exclusions:** no HST-1 through HST-19 changes; no connection snapshot/selection, onboarding/favorites, schema, execution, comparisons, STS, cache-format or policy redesign, Kusto discovery, protocol changes, browser/documents, or deferred ACT work.
