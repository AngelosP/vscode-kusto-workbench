...
**Next boundary:** migrate one standalone row-bearing export/publication workflow from mutable latest state to an exact artifact binding and a dedicated export decision; do not reuse `sendToModel` or `exposeToActiveContent`. Prove A-to-B pinning, explicit refresh/rebind, permission denial, and source revocation for that one workflow before expanding. Keep SQL exact provenance, SQL comparisons, coordinator convergence, and `ACT` outside the slice.
