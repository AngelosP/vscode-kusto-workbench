...
The comparison was refreshed against the current working tree after HST-11 Cached Values open application-handler closure. It includes the full implemented feature set described by the README, package contributions, persisted formats, architecture documentation, browser extension, and test suites.
...
| 1 | `HST` | Host application composition; retire `QueryEditorProvider` as an application shell | 4/4/5/4/4/5 | 47 | HST-1 through HST-11 closed; HST-12 selected |
...
**Why next:** HST remains the highest eligible gap. `openCopilotAgent` is the smallest coherent provider-owned route remaining: one payload-free message awaits the existing shared `openKustoWorkbenchAgentChat()` orchestration. Copilot generation, conversations, tools, dashboard prompts, editing preferences, Kusto/SQL workflows, and generic navigation are materially broader.

**Boundary:** extract only the provider's `openCopilotAgent` route into one injected, response-free `HostCopilotAgentOpenApplicationHandler`. Preserve the original message object, one awaited invocation of the existing helper, its current first-open/reapply/fallback semantics and 150 ms timing, resolved-value discard, synchronous decline of unrelated traffic, accepted settlement across disposal, suppression after idempotent disposal, and every existing message/protocol shape.

**Falsifiable hypothesis:** if one Copilot-agent-open handler owns the webview route, `QueryEditorProvider` can offer the exact message without importing or invoking `openKustoWorkbenchAgentChat`, while the existing shared helper remains the native chat-open orchestration used by extension commands, dashboard prompts, and Copilot workflows.

**Cheapest discriminating check:** construct the real provider with one final structural fake handler, send one exact `{ type: 'openCopilotAgent' }` object, and prove reference-identical forwarding while the shared chat-open helper remains untouched. Before injection the fake receives zero calls and the provider invokes the helper directly.

**Exclusions:** no HST-1 through HST-11 changes; no Copilot generation, conversation, tool, first-time, dashboard-prompt, or extension-command behavior; no helper command IDs/timing/fallback changes; no editing preferences, generic presentation/navigation framework, emitter, protocol, browser, document, deferred ACT, or Kusto/SQL lifecycle changes.
