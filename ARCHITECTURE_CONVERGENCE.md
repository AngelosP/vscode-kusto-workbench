...
| 1 | `HST` | Host application composition; retire `QueryEditorProvider` as an application shell | 4/4/5/4/3/5 | 46 | HST-1 through HST-26 closed; HST-27 selected |
...
**Post-HST-26 rescore:** independent review found no score change. HST remains the highest eligible gap at 46, ahead of PRO at 45, DOC at 43, BRW at 41, CMP at 39, DSH at 35, and KLS at 30; ACT remains deferred by product direction. The provider now owns 42 live discriminator cases across execution, schema, SQL/STS, comparisons, and Copilot.

**Boundary:** extract only the provider `clearCopilotConversation` route into one injected `HostCopilotConversationClearApplicationHandler`. Own exact original-object route admission, Kusto-versus-SQL branching, exact Kusto request-identity validation, and awaitable delegation to the existing Copilot clear capabilities. Append the constructor slot immediately after `copilotWriteQueryPreparationApplication`. `QueryEditorProvider` must not retain the discriminator branch or directly invoke either clear capability.
...
**Falsifiable hypothesis:** if one handler owns only conversation-clear admission, the real provider can reference-identically forward exact Kusto and SQL clear requests and await exact settlement without retaining the flavor/identity branch or directly mutating Copilot conversation state, while `CopilotService` remains the sole conversation-state owner.

**Cheapest discriminating check:** add `queryEditorProviderCopilotConversationClearHandler.test.ts` without importing the future module and append a structural fake immediately after the preparation constructor slot. Send exact identity-bearing Kusto and box-scoped SQL `clearCopilotConversation` objects through the real provider. Prove reference-identical forwarding, deferred settlement, exact rejection, injected-slot identity, and zero direct provider Copilot or transport effects. Record the intended zero-call failure before production edits.

**Exclusions:** no HST-1 through HST-26 changes; no conversation-state redesign, start/cancel write-query flow, SQL preflight, model selection/options/status, first-time UX, optimization, inline completion, execution, schema, STS, connection, comparison-summary/admission, protocol/browser, generic handler framework, document ownership, or deferred ACT work.
