...
| 1 | `HST` | Host application composition; retire `QueryEditorProvider` as an application shell | 4/4/5/4/3/5 | 46 | HST-1 through HST-27 closed; HST-28 selected |
...
**Post-HST-27 rescore:** independent review found no score change. HST remains the highest eligible gap at 46, ahead of PRO at 45, DOC at 43, BRW at 41, CMP at 39, DSH at 35, and KLS at 30; ACT remains deferred by product direction. The provider now owns 41 live discriminator cases across execution, schema, SQL/STS, comparisons, and Copilot.

**Selection rationale:** `removeFromCopilotHistory` is the next complete single-route ownership slice: `CopilotService` already owns normalization, history lookup, eligible entry kinds, and mutation. `copilotChatFirstTimeCheck` also has a thin provider branch, but its service method combines durable first-use state, modal UX, Agent Chat opening, and three response outcomes and merits a later complete workflow boundary rather than an admission-only wrapper. `clearComparisonSummary` mutates provider-owned state shared with comparison lifecycle and therefore cannot establish an independent owner through route extraction alone.

**Boundary:** extract only the provider `removeFromCopilotHistory` route into one injected `HostCopilotHistoryRemovalApplicationHandler`. Own exact original-object route admission and awaitable delegation to the existing Copilot history-removal capability. Append the constructor slot immediately after `copilotConversationClearApplication`. `QueryEditorProvider` must not retain the discriminator branch or directly invoke history removal.

**Preserved owners:** `CopilotService.removeFromCopilotHistory()` retains box/entry normalization, history lookup, eligible `tool-call` / `general-rules` entry decisions, and the `removed` mutation. The shared Kusto/SQL Copilot chat manager retains immediate UI removal and exact message emission. Conversation creation, clear semantics, first-time UX, start/cancel execution, optimization, response routing, and message shapes remain unchanged.

**Falsifiable hypothesis:** if one handler owns only history-removal admission, the real provider can reference-identically forward exact Kusto-box and SQL-box removal requests and await exact settlement without directly invoking or mutating Copilot history, while `CopilotService` remains the sole history-state owner.

**Cheapest discriminating check:** add `queryEditorProviderCopilotHistoryRemovalHandler.test.ts` without importing the future module and append a structural fake immediately after the HST-27 constructor slot. Send exact Kusto-box and SQL-box `removeFromCopilotHistory` objects through the real provider. Prove reference-identical forwarding, deferred settlement, exact rejection, injected-slot identity, and zero direct provider Copilot or transport effects. Record the intended zero-call failure before production edits.

**Exclusions:** no HST-1 through HST-27 changes; no history-model or conversation-state redesign, Kusto owner/flavor identity addition, first-time UX, conversation clearing, start/cancel write-query flow, optimization, execution, schema, STS, connection, comparison-summary/admission, protocol/browser, generic handler framework, document ownership, or deferred ACT work.
