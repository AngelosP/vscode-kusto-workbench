...
The comparison was refreshed against the current working tree after HST-12 Copilot Agent open application-handler closure. It includes the full implemented feature set described by the README, package contributions, persisted formats, architecture documentation, browser extension, and test suites.
...
| 1 | `HST` | Host application composition; retire `QueryEditorProvider` as an application shell | 4/4/5/4/4/5 | 47 | HST-1 through HST-12 closed; HST-13 selected |
...
**Why next:** HST remains the highest eligible gap. The paired `editorCursorPositionChanged` and development-only `getEditorCursorStatusSnapshot` routes form the next smallest coherent provider-owned presentation workflow: the provider still computes panel-scoped owner IDs, gates visibility/production behavior, mutates the shared status-bar adapter, shapes the snapshot response, and clears its owner prefix during panel transitions. Copilot availability is already delegated to `CopilotService`; editing preferences and Kusto/SQL workflows are materially broader.

**Boundary:** extract only `editorCursorPositionChanged`, `getEditorCursorStatusSnapshot`, panel-hidden clearing, and panel-disposal clearing into one injected `HostEditorCursorStatusApplicationHandler`. Preserve one unique provider owner prefix, exact box-ID/editor-kind trimming and fallback, visible-panel update admission, development-only snapshot response with exact request ID and hidden fallback, swallowed snapshot transport failures, stale-owner protection in `EditorCursorStatusBar`, and existing panel lifecycle ordering. Keep status-bar item creation, formatting, accessibility, global disposal, webview emitters, and message shapes unchanged.

**Falsifiable hypothesis:** if one cursor-status handler owns both inbound routes and panel clearing, `QueryEditorProvider` can forward the exact typed objects and panel visibility/disposal signals without retaining owner-ID, status update, snapshot response, or prefix-clear decisions, while the existing shared `EditorCursorStatusBar` remains the sole status-item/formatting owner.

**Cheapest discriminating check:** append one structural fake cursor-status handler to the real provider constructor, send exact update and snapshot objects, and prove reference-identical forwarding while the injected `EditorCursorStatusBar` and provider transport remain untouched. Before injection the fake receives zero calls and the provider directly updates the status bar and publishes the snapshot.

**Exclusions:** no HST-1 through HST-12 changes; no status-bar formatting/item lifecycle redesign; no cursor webview emitter or message-shape changes; no generic presentation framework; no Copilot, editing-preference, browser, document, deferred ACT, Kusto, or SQL lifecycle changes.
