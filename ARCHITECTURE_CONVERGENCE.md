...
| 1 | `HST` | Host application composition; retire `QueryEditorProvider` as an application shell | 4/4/5/4/4/5 | 47 | HST-1 through HST-16 closed; HST-17 selected |
...
**Why next:** HST remains the highest eligible gap. `promptAddSqlConnection` and `addSqlConnection` are one bounded provider-owned application path containing native prompt orchestration, credential-bearing manager input, profile selection persistence, and exact acknowledgement publication. Existing SQL connection, lifecycle, database/schema, execution, and Leave No Trace owners can remain unchanged.

**Boundary:** extract only `promptAddSqlConnection`, `addSqlConnectionFromWebview`, their complete current provider implementations, and their two provider cases into one injected `HostSqlConnectionOnboardingApplicationHandler`. Keep `SqlConnectionManager` as persistent connection/secret owner and keep the provider's revisioned, policy/principal-aware `sqlConnectionsData` publication unchanged. Preserve exact prompt options/order/cancellation, AAD and SQL Login inputs, trimming/defaults, manager call shape, `sql.lastConnectionId` ordering, and exact `sqlConnectionAdded` object.

**Falsifiable hypothesis:** if one SQL onboarding handler owns both routes, the real provider can reference-identically forward each exact object and await claimed work without directly invoking native prompts, SQL connection mutation, global-state selection storage, or transport, while all current cancellation, credential, persistence, and acknowledgement behavior remains unchanged.

**Cheapest discriminating check:** append one structural fake SQL onboarding handler to the real provider constructor, send exact `promptAddSqlConnection` and `addSqlConnection` objects, and prove ordered reference-identical forwarding while both legacy provider methods, native prompts, `SqlConnectionManager`, `globalState`, and provider transport remain untouched. Record the intended zero-call failure with `--maxWorkers=1` before production changes.

**Exclusions:** no HST-1 through HST-16 changes; no Kusto connection work, SQL connection update/remove/test, `getSqlConnections`, `saveSqlLastSelection`, favorites, databases/schemas, STS, auth override lifecycle, execution, Copilot, tools, browser, documents, generic connection framework, or deferred ACT work.
