...
The comparison was refreshed against the current working tree after HST-15 Kusto connection-intake application-handler closure. It includes the full implemented feature set described by the README, package contributions, persisted formats, architecture documentation, browser extension, and test suites.
...
| 1 | `HST` | Host application composition; retire `QueryEditorProvider` as an application shell | 4/4/5/4/4/5 | 47 | HST-1 through HST-15 closed; HST-16 selected |
...
**Why next:** HST remains the highest eligible gap. `promptAddConnection`, `addConnection`, and `testKustoConnection` form the next bounded provider-owned application path: canonical dialog opening, draft validation/testing, account-aware persistence, last-selection update, connection refresh, and exact UI terminals. Bulk/XML intake is now independently owned and does not need to move again.

**Boundary:** extract only `promptAddConnection`, `addConnectionFromWebview`, `testConnectionFromWebview`, their complete current `ConnectionService` implementations, and their three provider cases into one injected `HostKustoConnectionOnboardingApplicationHandler`. Keep `ConnectionManager` as persistent connection owner, `KustoAuthPreferenceService` as account-preference owner, `KustoQueryClient` as the test/data adapter, and the provider's revisioned `connectionsData` refresh as an injected effect. Keep dialog controls, message shapes, selection storage implementation, and database-list tracing unchanged.

**Falsifiable hypothesis:** if one onboarding handler owns all three routes, the real provider can reference-identically forward each exact object and await claimed work without directly invoking `ConnectionService`, refresh, transport, authentication, or database-test effects, while every current success/warning/error object and side-effect order remains unchanged.

**Cheapest discriminating check:** append one structural fake onboarding handler to the real provider constructor, send the exact prompt/add/test objects, and prove ordered reference-identical forwarding while the three existing `ConnectionService` methods, provider refresh, provider transport, auth preferences, and Kusto client remain untouched. Before injection the fake receives zero calls and the current branches execute directly.

**Exclusions:** no HST-1 through HST-15 changes; no favorites, bulk/XML intake, connection removal/update, auth lifecycle redesign, database/schema browsing, execution, selection behavior outside the existing add workflow, SQL, Copilot, tools, first launch, configuration, browser, document, generic connection framework, or deferred ACT work.
