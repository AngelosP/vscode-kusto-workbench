...
The comparison was refreshed against the current working tree after HST-20 SQL database discovery application-handler closure. It includes the full implemented feature set described by the README, package contributions, persisted formats, architecture documentation, browser extension, and test suites.
...
| 1 | `HST` | Host application composition; retire `QueryEditorProvider` as an application shell | 4/4/5/4/3/5 | 46 | HST-1 through HST-20 closed; HST-21 selected |
...
**Why next:** HST remains the highest eligible gap at 46. `kqlLanguageRequest` is the smallest remaining complete provider-owned request-to-terminal workflow: one route, one private method, one mature language-service dependency, and no authoritative mutable map. Nearby development-note, comparison-summary, and inline-completion candidates are more coupled to document mutation, comparison state, or Copilot execution.

**Boundary:** extract only `kqlLanguageRequest`, `handleKqlLanguageRequest`, request-ID/method/params admission, both `KqlLanguageServiceHost` calls, supported/unsupported/failure `kqlLanguageResponse` construction, logging, and disposal into one injected `HostKqlLanguageRequestApplicationHandler`. Append the constructor slot immediately after `sqlDatabaseDiscoveryApplication`.

**Preserved owners:** `KqlLanguageServiceHost` retains selection fallback, cached-schema/auth-partition resolution, and analysis delegation. `KqlLanguageService` retains diagnostics/reference algorithms; `extension.ts` retains independent text-editor diagnostics; the webview retains its timeout/resolver ledger and response application. Message shapes and the existing KQL language protocol remain unchanged.

**Falsifiable hypothesis:** if one handler owns the complete KQL request-to-terminal sequence, the real provider can reference-identically forward exact requests without directly invoking the language host, transport, or logger while preserving current settlement behavior.

**Cheapest discriminating check:** add `queryEditorProviderKqlLanguageRequestHandler.test.ts` without importing the future module, append a structural fake after the SQL discovery slot, send an exact `kusto/findTableReferences` request, and prove one reference-identical call, deferred settlement, exact rejection, and zero provider language-host/transport/logger effects. Record the intended zero-call failure with `--maxWorkers=1` before production edits.

**Exclusions:** no HST-1 through HST-20 changes; no analyzer decomposition, schema/cache/auth changes, protocol unification, webview timeout/bridge work, execution, Copilot, comparisons, persistence, documents/browser, generic handler framework, or deferred ACT work.
