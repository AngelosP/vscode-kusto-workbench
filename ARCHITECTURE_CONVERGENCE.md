...
The comparison was refreshed against the current working tree after HST-13 editor cursor-status application-handler closure. It includes the full implemented feature set described by the README, package contributions, persisted formats, architecture documentation, browser extension, and test suites.
...
| 1 | `HST` | Host application composition; retire `QueryEditorProvider` as an application shell | 4/4/5/4/4/5 | 47 | HST-1 through HST-13 closed; HST-14 selected |
...
**Why next:** HST remains the highest eligible gap. The three `setCaretDocsEnabled`, `setAutoTriggerAutocompleteEnabled`, and `setCopilotInlineCompletionsEnabled` routes form one complete provider-owned application-preference mutation workflow: the provider maps discriminators to storage keys, normalizes booleans, invokes the revisioned preference owner, and chooses global broadcast versus current-panel fallback. Connection/favorite paths are already delegated but materially broader; tool-state correlation owns timeouts plus SQL comparison reconciliation.

**Boundary:** extract only those three mutation routes and the current `updateEditingPreference` orchestration into one injected `HostEditingPreferencesApplicationHandler`. Keep `editingPreferences.ts` as persistence/revision/snapshot owner, keep initial preference projection in `connectionsData`, and preserve exact authoritative `editingPreferencesData` publication through all-webview broadcast with current-panel fallback. Do not change first-launch setup, configuration migration/listeners, webview application, or message shapes.

**Falsifiable hypothesis:** if one editing-preferences handler owns discriminator mapping, mutation, and publication, `QueryEditorProvider` can forward each exact typed object and await claimed work without retaining storage-key or broadcast decisions, while application-scoped revision and cross-window behavior remain unchanged.

**Cheapest discriminating check:** append one structural fake editing-preferences handler to the real provider constructor, send the three exact message objects, and prove reference-identical forwarding in order while `setEditingPreference`, orchestrator broadcast, and provider transport remain untouched. Before injection the fake receives zero calls and the provider performs the existing mutation path.

**Exclusions:** no HST-1 through HST-13 changes; no first-launch, configuration migration/listener, toolbar optimistic state, or initial-connections projection changes; no favorites, tool-state correlation, Copilot generation, generic settings framework, browser, document, deferred ACT, Kusto, or SQL lifecycle changes.
