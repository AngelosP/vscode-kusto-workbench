...
| 1 | `HST` | Host application composition; retire `QueryEditorProvider` as an application shell | 4/4/5/4/3/5 | 46 | HST-1 through HST-25 closed; HST-26 selected |
...
**Post-HST-25 rescore:** independent review found no score change. HST remains the highest eligible gap at 46, ahead of PRO at 45, DOC at 43, BRW at 41, CMP at 39, DSH at 35, and KLS at 30; ACT remains deferred by product direction. The provider now owns 43 live discriminator cases across execution, schema, SQL/STS, comparisons, and Copilot.

**Boundary:** extract only the provider `prepareCopilotWriteQuery` route into one injected `HostCopilotWriteQueryPreparationApplicationHandler`. Own exact original-object route admission and awaited delegation to the existing Copilot capability. Append the constructor slot immediately after `copilotAvailabilityApplication`. `QueryEditorProvider` must not retain the discriminator branch or directly invoke Copilot preparation work.

**Preserved owners:** `CopilotService` retains box-ID normalization, model discovery/filtering/sorting and label formatting, preferred/default model selection, the persisted last-model read, Kusto/SQL local-tool selection, unavailable status, and exact `copilotWriteQueryOptions` / `copilotWriteQueryStatus` construction/publication. Kusto and SQL chat-manager callers, response routing/application, model execution, conversations/history, first-time UX, and message shapes remain unchanged.

**Falsifiable hypothesis:** if one handler owns only Copilot write-query preparation admission, the real provider can reference-identically forward the exact request and await exact settlement without retaining the discriminator branch, directly invoking Copilot, or publishing options/status, while `CopilotService` remains the sole preparation/result owner.

**Cheapest discriminating check:** add `queryEditorProviderCopilotWriteQueryPreparationHandler.test.ts` without importing the future module and append a structural fake immediately after the availability constructor slot. Send exact Kusto and SQL `prepareCopilotWriteQuery` objects through the real provider. Prove reference-identical forwarding, deferred settlement, exact rejection, injected-slot identity, and zero direct provider Copilot or transport effects. Record the intended zero-call failure before production edits.

**Exclusions:** no HST-1 through HST-25 changes; no first-time UX, conversations/history, model selection/options/status semantics, optimization, inline completion, execution, schema, STS, connection, comparison-summary/admission, protocol/browser, generic handler framework, document ownership, or deferred ACT work.
