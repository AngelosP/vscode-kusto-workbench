...
| 1 | `HST` | Host application composition; retire `QueryEditorProvider` as an application shell | 4/4/5/4/3/5 | 46 | HST-1 through HST-23 closed; HST-24 selected |
...
**Post-HST-23 rescore:** all gap scores remain unchanged. HST remains the highest eligible gap at 46, ahead of PRO at 45, DOC at 43, BRW at 41, CMP at 39, DSH at 35, and KLS at 30; ACT remains deferred by product direction. The provider still owns a live inline-completion admission route, while comparison-summary waiter state has no active runtime caller and is not selected merely because maps remain.

**Boundary:** extract only the provider `requestCopilotInlineCompletion` route into one injected `HostCopilotInlineCompletionApplicationHandler`: exact original-object admission, Kusto delegation, SQL owner-token assertion, SQL delegation with the exact issued owner/token, and the existing empty `copilotInlineCompletionResult` fallback when SQL admission fails. Append the constructor slot immediately after `developmentNoteMutationApplication`. `QueryEditorProvider` supplies narrow callbacks and must not retain the discriminator branch or fallback construction.

**Preserved owners:** `CopilotService` retains request normalization, model selection/cache, KQL/T-SQL prompt construction, context trimming, eight-second cancellation, replacement, model streaming, completion cleanup, and normal result construction. SQL lifecycle/workbench services retain owner-token issuance, canonical principal/privacy admission, and revalidation. The webview retains request IDs, its ten-second resolver, Monaco spinner/application, cancellation policy, editing-preference admission, and unchanged message shapes.

**Falsifiable hypothesis:** if one handler owns only inline-completion route admission, the real provider can reference-identically delegate both Kusto and SQL requests without retaining flavor branching, owner-token assertion, or SQL fallback construction, while `CopilotService` and SQL lifecycle remain the sole model and owner authorities.

**Cheapest discriminating check:** add `queryEditorProviderCopilotInlineCompletionHandler.test.ts` without importing the future module and append a structural fake immediately after the development-note constructor slot. Send exact Kusto and SQL `requestCopilotInlineCompletion` objects through the real provider. Prove reference-identical forwarding, deferred settlement and exact rejection, injected-slot identity, and zero direct provider owner-token, Copilot-service, or fallback-transport effects. Record the intended zero-call failure before production edits.

**Exclusions:** no HST-1 through HST-23 changes; no Copilot model/prompt/cache/streaming rewrite, SQL owner/privacy authority migration, webview resolver/spinner change, editing-preference change, comparison-summary work, document ownership, Kusto/SQL execution, protocol/browser changes, generic handler framework, or deferred ACT work.
