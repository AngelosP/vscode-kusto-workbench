...
| 1 | `HST` | Host application composition; retire `QueryEditorProvider` as an application shell | 4/4/5/4/3/5 | 46 | HST-1 through HST-24 closed; HST-25 selected |
...
**Post-HST-24 rescore:** independent review found no score change. HST remains the highest eligible gap at 46, ahead of PRO at 45, DOC at 43, BRW at 41, CMP at 39, DSH at 35, and KLS at 30; ACT remains deferred by product direction. The provider still owns 44 live discriminator cases across execution, schema, SQL/STS, comparisons, and Copilot. Comparison-summary waiter state still has no active runtime caller and is not selected merely because maps remain.

**Boundary:** extract only the provider `checkCopilotAvailability` route into one injected `HostCopilotAvailabilityApplicationHandler`. Own exact original-object route admission and awaited delegation to the existing Copilot capability. Append the constructor slot immediately after `copilotInlineCompletionApplication`. `QueryEditorProvider` must not retain the discriminator branch or directly invoke Copilot availability work.

**Preserved owners:** `CopilotService` retains `vscode.lm.selectChatModels({ vendor: 'copilot' })`, availability/failure-to-false semantics, and exact `copilotAvailability` construction/publication. The global startup, Kusto section, and SQL section webview callers retain when and for which `boxId` availability is requested. Response routing, UI application, model lifecycle, conversations, inline completion, editing preferences, and message shapes remain unchanged.

**Falsifiable hypothesis:** if one handler owns only Copilot availability route admission, the real provider can reference-identically forward the request and await exact settlement without retaining the discriminator branch, directly invoking Copilot, or publishing a response, while `CopilotService` remains the sole model-discovery and result owner.

**Cheapest discriminating check:** add `queryEditorProviderCopilotAvailabilityHandler.test.ts` without importing the future module and append a structural fake immediately after the inline-completion constructor slot. Send the exact `checkCopilotAvailability` object through the real provider. Prove reference-identical forwarding, deferred settlement, exact rejection, injected-slot identity, and zero direct provider Copilot or transport effects. Record the intended zero-call failure before production edits.

**Exclusions:** no HST-1 through HST-24 changes; no first-time UX, conversation/history, optimization, inline-completion, execution, schema, STS, connection, comparison-summary, protocol/browser, generic handler framework, document ownership, or deferred ACT work.
