...
| `queryEditorProvider.ts` | Main webview transport/composition adapter. Routes host↔webview messages and retains SQL query/language, persistence, and connection UX adapters |
| `mainWebviewStartupGateway.ts` | Panel-scoped startup transport for native KQLX plus KQL and SQL compatibility editors. Owns listener-first installation, explicit dispatcher readiness, ordered startup queues, correlated reply reentrancy, and disposal retirement |
...
| `editingPreferencesApplicationHandler.ts` | Injected host application owner for the three editing-preference mutation routes, exact storage-key mapping and boolean normalization, revisioned mutation delegation, live all-webview publication with current-panel fallback, and disposal |
...
Editing-preference mutation remains outside the document-view protocol. `HostEditingPreferencesApplicationHandler` is the sole provider-route owner of `setCaretDocsEnabled`, `setAutoTriggerAutocompleteEnabled`, and `setCopilotInlineCompletionsEnabled`. It maps each discriminator to the existing `STORAGE_KEYS` key, normalizes with `!!message.enabled`, awaits exactly one `setEditingPreference`, and publishes that exact authoritative `editingPreferencesData` object through the live tool orchestrator or awaited current-panel fallback. `QueryEditorProvider` only constructs or accepts the handler, forwards each original typed object, awaits claimed work, and disposes it. `editingPreferences.ts` retains application configuration, persistence, timestamp-monotonic revisions, snapshots, and migration; `extension.ts` retains configuration-change refresh; and initial preference projection remains part of `connectionsData`. Disposal suppresses later requests without changing already accepted mutation or publication settlement.
...
Copilot inline-completion admission remains outside the document-view protocol. `HostCopilotInlineCompletionApplicationHandler` is the sole provider-route owner of `requestCopilotInlineCompletion`: it receives the original typed object, distinguishes Kusto from SQL, asserts SQL owner-token admission through the existing lifecycle capability, delegates with the exact issued owner and token, preserves the existing empty `copilotInlineCompletionResult` fallback for SQL admission or delegation failure, and owns disposal. `QueryEditorProvider` only constructs or accepts the handler in the constructor slot immediately after development-note mutation, reference-identically offers requests, awaits claimed work, supplies narrow callbacks, and disposes it. `CopilotService` retains request normalization, model selection/cache, prompts, context trimming, eight-second cancellation, replacement, streaming, owner revalidation, cleanup, and normal result construction. SQL lifecycle/workbench services retain owner-token, principal, privacy, and dispatch authority. The webview retains request IDs, its ten-second resolver, Monaco spinner/application, cancellation policy, editing-preference admission, and message shapes.
...
| `editingPreferencesApplicationHandler.test.ts` | Exact preference key mapping, boolean normalization, authoritative publication identity, live orchestrator and current-panel paths, failures, synchronous decline, and disposal contracts |
...
| `kqlLanguageRequestApplicationHandler.test.ts` | Exact diagnostics/reference delegation, request admission, malformed runtime fallback, unsupported/failure responses, unawaited transport, logger failure, synchronous decline, and disposal contracts |
...
| `developmentNoteMutationApplicationHandler.test.ts` | Exact request IDs/outbound mutation, unavailable and delivery failures, five-second timeout, claimed response shaping, synchronous decline, late response behavior, and disposal settlement |
...
| `copilotInlineCompletionApplicationHandler.test.ts` | Exact Kusto/SQL route admission, issued SQL owner/token delegation, empty fallback, transport settlement, rejection, synchronous decline, and disposal contracts |
...
| `queryEditorProviderCopilotQueryWorkflowHandler.test.ts` | Red-first real-provider five-route reference-identical forwarding, deferred settlement, exact rejection, zero direct Copilot/preflight/token/transport effects, current 13-case provider inventory, and disposal ownership |
...
| `queryEditorProviderKustoSectionExecutionHandler.test.ts` | Red-first real-provider seven-route reference-identical forwarding, deferred settlement, exact rejection, zero direct coordinator/client/selection/transport/acknowledgement/Copilot-close effects, 13-case inventory, and disposal ownership |
...
| `queryEditorProviderComparisonPreparationHandler.test.ts` | Red-first real-provider three-route and preparation forwarding, deferred settlement, exact rejection, zero direct map/lifecycle/broker/coordinator/Copilot effects, 13-case inventory, and disposal ownership |
...
| `queryEditorProviderSqlSectionExecutionHandler.test.ts` | Red-first real-provider execute/cancel forwarding, constructor identity, deferred settlement, exact rejection, zero direct lifecycle/broker/workbench/client/transport effects, 13-case inventory, and disposal ownership |
