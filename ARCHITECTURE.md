...
Legacy id-less sections receive stable opaque IDs per raw document revision before webview projection, and the same projected baseline is used for persistence. Every host projection carries a source generation and becomes active only after the webview acknowledges successful section materialization; persistence echoes that generation. KQL/SQL compatibility sidecars deterministically materialize IDs after overlay, enrich edits from the exact locked baseline before privacy sanitation, and use CAS/retry for writes, repair, and recovery. Their accepted physical identity is captured during load or returned by the locked creation/adoption publication itself, then retained through later draft writes. Privacy sanitation removes explicitly modeled row-bearing fields without granting authority over future extensions. Linked queries support one plain `.kql`/`.csl` target, use exact structural ownership, reject self/non-query targets, fence asynchronous loads by document generation, and reject stale or previously missing linked-file writes until reload establishes an exact baseline. Native linked Save holds one serialized content lease across buffer mutation and durable publication, while rollback treats buffer and disk as independent CAS surfaces so a newer direct edit is never overwritten.

Native notebook Markdown is host-owned. `markdownSectionDefinition.ts` owns persisted validation and patch semantics, while `MarkdownDocumentAggregate` owns ordered application state, document revisions, per-section revisions, and add/patch/remove transitions. The webview `markdown-document-client.ts` sends revisioned commands and renders complete host projections; `kw-markdown-section` remains a view and never supplies native Markdown persistence through `serialize()`. Plain `.md` compatibility remains primary-text-owned and retains its compatibility serializer.

Each normalized notebook URI has one physical Markdown queue shared before and after projection acknowledgement, across transient panels, owner replacement, Save, rollback, close, and reopen. A projection candidate becomes canonical only after a live reload request, exact source/generation checks, and application acknowledgement. Every valid command ID receives one terminal result. Native Save converts the webview barrier into a queue lease held through the matching non-canonical commit; rejected barriers abort, overlapping Saves serialize, canonical result-restoration Saves cannot release user leases, and disposal settles accepted leases. Live panel ownership is distinct from retained aggregate state: canonical-panel disposal reprojects a survivor without rewinding revisions, while text-document cleanup deletes retained owner and queue only after the tail stabilizes and an atomic final check confirms that the URI did not reopen. Final serialization still overlays the aggregate snapshot onto the exact current codec baseline, preserving COD-1 future data and order.
...
The extension host opens the native picker, creates a one-use nonce, and asks the webview to provide the projection. The webview revalidates the active table and creates a short-lived `csv:<boxId>:save` binding before returning bytes. One table may retain one projection, with eight pending projections globally. The host accepts only the matching nonce, box, and artifact, consumes it once, retains a bounded completed-intent tombstone against request replay, and writes immediately to the selected URI. Replacement, transitive result revocation, removal, every applied document transition (including malformed replacement), and panel disposal clear both sides and hide Save through a token-scoped release event. Picker cancellation or the visible 60-second transfer timeout cancels only that attempt, leaving an otherwise valid table retryable. URL-imported CSV uses `saveImportedCsv`; Connection Manager preview export retains its separate protocol.
