...
| `queryEditorProvider.ts` | Main webview transport/composition adapter. Routes host↔webview messages and retains cross-language query, Copilot, persistence, connection UX, URL, comparison, and imported-CSV adapters |
...
* **Host → Webview:** `QueryEditorProvider` owns panel transport; injected application handlers such as `HostDashboardApplicationHandler`, `HostArtifactCsvSaveApplicationHandler`, and `HostPythonExecutionApplicationHandler` send through that transport callback.
