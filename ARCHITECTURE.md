...
| `queryEditorProvider.ts` | Main webview transport/composition adapter. Routes host↔webview messages and retains cross-language query, Copilot, persistence, connection UX, and comparison adapters |
...
| `queryEditorProviderCopilotQueryWorkflowHandler.test.ts` | Red-first real-provider five-route reference-identical forwarding, deferred settlement, exact rejection, zero direct Copilot/preflight/token/transport effects, 25-case provider inventory, and disposal ownership |
