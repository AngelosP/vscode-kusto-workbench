...
* **Host → Webview:** `QueryEditorProvider` owns panel transport; injected application handlers such as `HostDashboardApplicationHandler`, `HostArtifactCsvSaveApplicationHandler`, `HostPythonExecutionApplicationHandler`, `HostQuerySharingApplicationHandler`, `HostUrlContentApplicationHandler`, `HostControlCommandSyntaxApplicationHandler`, and `HostResourceUriApplicationHandler` send through that transport callback. The response-free `HostCopilotContentOpenApplicationHandler`, `HostInformationNotificationApplicationHandler`, and `HostCachedValuesOpenApplicationHandler` own their native presentation effects without introducing a host message.
